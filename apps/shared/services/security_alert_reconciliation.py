...
class SQLAlchemySecurityAlertReconciliationRepository:
    ...
    def scan_security_alert_audits(self, *, started_at: datetime):
        return (
...
def reconcile_security_alert_batch(
    ...
    *,
    ...
    audits = repository.scan_security_alert_audits(
        started_at=_scan_started_at(watermark, overlap)
    )
    for audit in audits:
...
def _scan_started_at(watermark, overlap: timedelta):
    if watermark.cursor_occurred_at is None:
        return watermark.activation_started_at
    return max(
        watermark.activation_started_at,
        watermark.cursor_occurred_at - overlap,
    )
