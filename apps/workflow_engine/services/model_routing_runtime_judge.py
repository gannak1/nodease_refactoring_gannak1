    ...
    def _messages(
        ...
        *,
        ...
        instruction = (
            ...
            "세 축을 각각 0(없음)부터 3(높음)까지 내부적으로 채점하세요. capability_tier가 economy인 후보는 세 축의 최고점이 1 이하일 때만 선택할 수 있습니다. "
            "최고점이 2이면 balanced 이상의 후보를, 최고점이 3이면 advanced 이상의 후보를 선택하세요. 이 기준은 후보 중 정확한 모델 하나를 고르기 위한 Judge의 내부 판단 규칙입니다. "
            "같은 최소 등급 안에 여러 후보가 남을 때만 실제 계약 성적, 가격, 지연을 비교해 하나를 선택하세요. "
            "후보 목록의 순서, 모델 ID의 숫자, capability_tier 하나만으로 선택하지 말고 모든 후보를 비교하세요. 가격·지연·실패율은 비슷한 능력의 후보 사이에서 선택하는 보조 기준으로만 사용하세요. "
