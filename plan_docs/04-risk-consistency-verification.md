...
- 기존 Moduly 명세: `local/docs`
- Nodease 방향 문서: `local/nodease_docs`
...
| 항목 | 근거 |
| --- | --- |
| App을 project boundary로 쓰는 계획 | `local/docs/01-project-overview.md`의 App 중심 구조와 `workflow_id`, `active_deployment_id` |
| Workflow를 canvas로 보는 계획 | `local/docs/sections/04-frontend-screens.md`의 `/modules/[id]` 워크플로우 편집기 |
...
| trigger mode 정합성 리스크 | `local/docs/sections/03-runtime-architecture.md`, `06-data-model.md`, `07-workflow-engine.md`의 주의사항 |
| migration/create_all 리스크 | `local/docs/sections/03-runtime-architecture.md`의 Gateway startup 흐름 |
| 노드 단위 RBAC 후순위 | `local/nodease_docs/08-interpretation-audit.md`의 해석 점검 |
...
현재 `local/plan_docs`는 최종 ERD 문서(`local/erd_docs/02-final-mvp-erd-entities.md`, `local/erd_docs/03-permission-matrix.md`)와 같은 기준을 사용한다. `roles`, `user_roles`, polymorphic `resource_permissions`, `audit_events`, `rag_retrieval_traces`는 최종 MVP에서 새로 만들지 않는다. 권한은 dev baseline의 organization/team permission과 resource별 user direct permission으로, audit은 `audit_logs`로, RAG trace는 `trace_payloads`/metadata로 처리한다.
