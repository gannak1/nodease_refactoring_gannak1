# Nodease Permission Matrix 설계서

## 목적

이 문서는 기존 `main` 브랜치 대비 `dev` 브랜치에서 추가된 조직/팀 권한 ERD를 기준으로, 최종 MVP에서 사용할 권한 Matrix를 정의한다.

이 문서는 `local/erd_docs/02-final-mvp-erd-entities.md`를 따른다. 따라서 `roles`, `user_roles`, polymorphic `resource_permissions`를 새로 만들지 않는다. 기본 권한은 `organization`, `teams`, `team_memberships`, `team_*_permissions`를 기준으로 판정하고, 예외적 추가 권한은 resource별 `user_*_permissions`로 부여한다.
...
| `auth_state` 단일 상태 | dev ERD는 permission boolean set이 아니라 `auth_state` 문자열 하나를 가진다. |
...
dev DB는 `auth_state`를 string으로만 저장한다. 최종 MVP에서는 아래 값을 application-level 표준으로 사용한다.
...
- 이 문서의 `auth_state` 표준값은 최종 MVP 기준으로 확정한다.
...
- dev ERD에는 document별 permission table이 없다.
...
- dev ERD에는 `llm_model` 전용 team permission table이 없다.
...
기준 table: 없음. `apps`는 project boundary지만 dev ERD에는 app 전용 team permission table이 없다.
...
- 현재 문서에서는 dev ERD 보존 조건 때문에 `team_app_permissions`를 만들지 않는다.
...
기준 table: 없음. dev ERD에는 connection 전용 team permission table이 없다.
...
현재 dev ERD에는 explicit deny column이 없다. 최종 MVP에서도 user direct deny를 도입하지 않는다. 따라서 기본 우선순위는 아래와 같다.
...
| connection 전용 permission table | 최종 MVP에서는 만들지 않는다. 독립 connection 공유가 필요해질 때만 `team_connection_permissions`/`user_connection_permissions`를 별도 schema extension으로 검토한다. |
