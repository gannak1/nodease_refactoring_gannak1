...
def test_workflow_correlation_migration_backfills_only_valid_existing_refs(
    ...
    try:
        ...
        connection.execute(
            text("CREATE TABLE workflow_runs (id UUID PRIMARY KEY)")
        )
        connection.execute(
            text(
        ...
        connection.execute(
            text("INSERT INTO workflow_runs (id) VALUES (:id)"),
            {"id": workflow_run_id},
        )
        connection.execute(
            text(
