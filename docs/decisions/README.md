...
| [ADR-0006](ADR-0006-accept-rbac-auth-state-and-user-direct-permission.md) | Accepted | RBAC auth_state 및 User Direct Permission 승인 | 현재 workflow/LLM credential RBAC 기준이며 MBA-176에서 `user_knowledge_permissions`를 같은 additive allow 모델로 확장한다. `user_audit_permissions`는 아직 구현되지 않음 |
