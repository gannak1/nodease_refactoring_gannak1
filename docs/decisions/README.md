...
| [ADR-202606290131-audit-action-naming-standard](ADR-202606290131-audit-action-naming-standard.md) | Accepted | Audit action naming 표준 | MVP 1 주요 `AuditAction`과 organization membership invite/accept/update/remove action 구현. permission row 변경은 `*_permission.created/updated/deleted` data-change audit도 기록. `policy.warn`, `policy.block`, `rag.retrieve`는 MVP 2 목표 action |
