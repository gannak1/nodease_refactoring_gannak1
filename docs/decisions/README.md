...
| [ADR-0021](ADR-0021-webhook-capture-helper-security-boundary.md) | Accepted | Webhook capture helper security boundary | Capture start/status는 public trigger 실행 표면이 아니라 로그인 사용자 + target workflow `deploy` 권한이 필요한 디버그 helper다. App secret 인증만으로는 capture를 시작/조회할 수 없고, captured payload는 raw 원문이 아니라 redacted/capped preview만 반환한다 |
