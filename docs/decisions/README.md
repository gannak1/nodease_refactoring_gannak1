...
| [ADR-0022](ADR-0022-incremental-hexagonal-architecture-adoption.md) | Accepted | 헥사고널 아키텍처 점진 도입 | 현재 코드는 Gateway deployment domain package scaffold만 추가했다. 신규/refactor backend flow의 작성 기준은 router/use case/domain policy/port/adapter 경계이며, 다른 domain package는 각 도메인의 첫 리팩터링 PR에서 실제 use case/port와 함께 만든다. Deployment preflight는 후속 pilot PR에서 이관이 완료된 뒤 reference implementation으로 사용한다. 기존 router/service 대량 이동은 하지 않는다 |
