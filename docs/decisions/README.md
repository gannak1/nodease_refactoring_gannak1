...
| [ADR-0023](ADR-0023-audit-actor-access-management-boundary.md) | Accepted | Audit actor 기반 organization access management 경계 | Audit `auditor`는 조회 전용이고 member/permission mutation은 organization manager만 수행한다. Suspension은 grant를 보존하며 direct restore는 explicit re-grant다. Access mutation과 canonical audit은 같은 DB transaction을 사용한다. ADR-0022의 deployment preflight reference pilot은 완료됐고, 실제 access-management use case/API/UI 구현은 아직 반영 전이다 |
