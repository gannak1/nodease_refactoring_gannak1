...
- 검색은 existing parent chunk embedding에 대한 cosine vector search만 사용한다. BM25, PostgreSQL FTS, keyword retrieval, keyword/vector hybrid, RRF, L2 score fusion, LLM 생성 문서 요약 및 summary embedding은 이 결정의 범위가 아니다.
...
- 전체 semantic retrieval deadline은 10초다. 각 cohort의 embedding timeout은 남은 시간과 4초 중 작은 값, PostgreSQL statement timeout은 남은 시간과 2초 중 작은 값이다. Cohort 우선순위는 후보 수 내림차순 뒤 model/dimension의 안정 순서다. 늦게 완료된 취소·stale 결과는 적용하지 않는다. 새 public 또는 cross-layer cancellation-token API는 추가하지 않는다. Agent Builder 내부의 live predicate는 동일 Gateway process의 bounded TTL/count cancellation marker만 확인하고 DB connection이나 SQL을 소비하지 않으며, 다른 process의 늦은 적용은 기존 request-status CAS가 차단한다.
...
- Retrieval port의 KB별 score projection은 권한 확인된 KB ID, bounded score 및 성공·degraded 상태만 반환한다. 별도로 action audit을 만들기 위한 bounded aggregate telemetry는 authorized candidate snapshot의 candidate/result/cohort/metadata-fallback/failed-cohort count bucket, timeout/failure 분류 및 latency bucket만 반환할 수 있다. 이 telemetry는 hidden 또는 denied candidate의 ID, 이름, exact count나 존재 여부를 포함하지 않는다. Parent 원문, document/chunk ID, embedding vector, provider request/response payload는 RecommendationService, Planner, Client, audit, trace 또는 log 경계로 전달하지 않는다.
- 이 조회는 일반 RAG answer retrieval이 아니므로 citation trace나 `rag.retrieve` payload를 만들지 않는다. 운영 관측은 위 allowlist를 넘는 후보 identity·내용·score·hidden candidate detail을 포함하지 않는다.
