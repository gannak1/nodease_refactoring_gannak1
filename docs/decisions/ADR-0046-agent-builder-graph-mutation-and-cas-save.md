...
CAS 저장은 workflow write 권한만 다시 확인하는 경계가 아니다. Server는 final candidate graph와 Catalog v3 schema에서 `resource_ref`, `credential_ref`, Knowledge/Collection binding과 WorkflowNode `appId`/`workflowId` 등 모든 resource reference를 직접 추출한다. 각 reference는 server-owned resolver로 현재 organization, 존재/lifecycle, required relation과 use/read/write 권한을 같은 DB transaction 안에서 재검증한다. Client가 보낸 reference inventory나 mutation 발급 시점의 allow 판정을 권위값으로 사용하지 않는다. Resource-bearing field에 등록된 resolver가 없거나 reference가 삭제·비활성·권한 회수·relation 변경 상태면 전체 save와 audit를 rollback하고 resource hiding을 적용한 오류로 닫는다. Safe operation envelope에 resource identifier나 revision snapshot을 추가하지 않으며 저장 시점의 authoritative row와 permission state가 기준이다. 이 규칙은 `initial_graph|graph_edit|replace_workflow|parameter_update|knowledge_binding`과 quick/guided/structure-only 모두에 동일하게 적용한다.
