...
Status: Accepted
Related ADRs: [ADR-0019](ADR-0019-agent-builder-preview-apply-save-boundary.md), [ADR-0021](ADR-0021-webhook-capture-helper-security-boundary.md)
...
현재 구현된 다음 16개 node type을 Agent Builder allowlist에 포함한다.
...
Agent Builder가 외부 action node를 draft에 포함하더라도 draft 생성, Preview Mode, apply/save는 해당 action을 실행하지 않는다. Credential, token, password, repository, channel, URL, target workflow처럼 자동으로 안전하게 확정할 수 없는 값은 원문을 생성하거나 추론하지 않고 빈 값과 `configuration_state=unresolved`로 남긴다. 저장된 workflow의 실제 실행은 기존 editor/runtime validation과 별도 사용자 동작을 거쳐야 한다.

Catalog에 등록되지 않은 임의 node type과 `implemented=false` node type은 Agent Builder allowlist에 포함하지 않는다.
...
- 모든 구현 node를 draft에 제안할 수 있지만 외부 action과 secret 사용은 자동 승인되지 않는다.
