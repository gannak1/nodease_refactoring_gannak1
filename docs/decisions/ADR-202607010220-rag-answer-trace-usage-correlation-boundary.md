...
RAG Agent answer 실행의 기준 record는 RAG 도메인이 소유하는 `rag_answer_runs`로 둔다. `rag_answer_runs`는 organization, actor, target KB, answer status, redaction-safe retrieval summary, citation summary, answer summary, top-level answer hash, policy result, generation summary, denormalized usage snapshot, `correlation_id`를 저장하는 실행 anchor다.
