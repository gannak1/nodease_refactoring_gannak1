...
Lease claim은 같은 provider attempt에 idempotent하다. Provider adapter는 outbound 호출 직전에 Memory context-attempt marker와 ADR-0069의 canonical usage `provider_started`를 [ADR-0071](ADR-0071-conversation-memory-runtime-admission-and-provider-fence.md)의 순서로 각각 durable하게 기록한다. Memory marker는 lease lifecycle만 보호하고 독립 send authority가 아니며, 실제 전송 권한과 no-replay 판단은 usage ledger가 소유한다. Claim 뒤 두 marker 전 crash는 claim expiry 후 새 lease/attempt로 재승인할 수 있다. Memory marker만 있고 usage가 exact `intent`인 partial commit은 current owner가 모든 binding을 다시 검증한 뒤 같은 attempt의 usage start까지만 진행할 수 있다. Usage가 `provider_started` 또는 outcome-unknown/terminal이면 provider를 자동 재호출하지 않고 reconciliation 또는 safe node failure로 닫는다. Permission/source invalidation event는 관련 entry, summary, materialization plan과 미사용/claimed-not-started lease를 무효화할 수 있지만 read-time current authorization을 대체하지 않는다. 외부 provider 호출과 permission mutation의 완전한 원자성은 보장하지 않으며 lease로 stale authorization window를 제한한다.
