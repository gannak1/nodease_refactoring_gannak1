...
9. Docker Compose는 application/internal dependency network를 `internal`로 만들고, Gateway·Knowledge·Workflow는 각각 proxy client network만 추가로 사용한다. Squid와 각 proxy client network의 DNS-only resolver만 별도 egress-capable network에 연결할 수 있으며, resolver는 host port를 publish하지 않고 fixed internal address에서 Docker service name과 public DNS만 전달한다. Target workload는 resolver 또는 Squid의 egress network에 직접 연결하지 않으며 proxy port도 host에 publish하지 않는다.
