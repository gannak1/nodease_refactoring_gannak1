...
18. Proxy-only workload가 첫 요청에서 외부 package 자산을 내려받지 않도록 Gateway/Knowledge와 Workflow image는 사용하는 tiktoken encoding, NLTK corpus, runtime과 동일한 immutable E5 `(model_id, revision)` snapshot과 build에서 활성화한 CrossEncoder 모델을 build 단계에 포함한다. Workflow image는 build에 사용한 E5 identity를 runtime 기본 환경으로 함께 고정하며 model ID 또는 revision override는 두 경계를 동시에 바꾸는 custom image build로만 허용한다. Runtime은 NLTK 또는 활성 모델 download를 호출하지 않으며 자산 누락 시 keyword 부가기능만 bounded warning으로 생략한다. 필수 tokenizer 또는 활성 reranker 자산이 없는 image는 배포 계약 실패다. Demo seed의 runtime embedding도 bare provider SDK 대신 shared operation-bound guarded client를 사용한다.
