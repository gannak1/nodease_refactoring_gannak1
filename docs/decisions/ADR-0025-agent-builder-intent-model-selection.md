...
Agent Builder는 사용자 자연어를 안전한 의미 후보로 구조화하기 위해 내부 LLM intent planner를 사용한다. 기존 구현은 provider별 고정 저비용 모델 이름을 순회해 사용할 credential/model 조합을 내부에서 자동 선택했다. 이 방식은 실제 사용 모델을 사용자가 확인하거나 변경할 수 없고, 조직에 허용된 최신 모델 목록과도 쉽게 어긋난다.
...
ADR-0019의 workflow node credential 자동 선택 금지는 유지한다. 이 ADR은 오직 Agent Builder 내부 intent planner runtime에 대한 좁은 예외이며, generated workflow node의 credential 또는 target을 자동 설정하지 않는다.
...
- Workflow node의 LLM model/credential 설정을 자동으로 채우지 않는다.
