...
Status: Accepted
Related ADRs: [ADR-0022](ADR-0022-incremental-hexagonal-architecture-adoption.md), [ADR-0040](ADR-0040-agent-builder-unified-model-recommendation.md), [ADR-0045](ADR-0045-agent-builder-direct-edit-parameter-guidance.md)

## 배경

Agent Builder는 자연어 요청을 구조화하는 최초 planner 호출과, schema 또는 의미 검증에 실패한 결과를 고치는 repair 호출을 최대 한 번 수행한다. 두 호출은 실제 LLM 비용을 발생시키지만 기존 workflow node 실행이 아니어서 `llm_usage_logs`에 기록되지 않았다. 이 누락은 조직 관리 비용, workflow별 비용과 예산, 내 모듈 월 예상 비용을 실제보다 낮게 만든다.
...
4. Provider 호출 전에 request/session/App primary workflow, credential 유효 상태, active chat model, verified credential-model 관계, 사용자 credential use 권한과 active organization membership을 검증하고 같은 attempt의 `pending` 사용량 행을 짧은 별도 transaction으로 확보한다. 이때 model·credential ID와 가격을 고정한다. 같은 identity가 이미 예약됐거나 성공으로 끝났다면 provider를 다시 호출하지 않고 fail-closed한다.
...
- 응답 schema가 잘못돼도 provider 응답에 검증 가능한 usage가 있으면 비용 기록은 보존된다.
