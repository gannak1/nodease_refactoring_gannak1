...
4. 검증 결과 중 선택한 IP를 실제 TCP destination으로 사용한다. Custom httpcore network backend는 원래 hostname을 다시 DNS 조회하지 않고, 연결 peer가 선택한 IP와 정확히 일치하는지 request byte 전송 전에 확인한다. 불일치하면 stream을 닫고 요청을 보내지 않는다.
