...
- 표준 `NetworkPolicy`는 CNI 집행이 없거나 같은 pod를 선택하는 별도 allow-all egress policy가 있으면 제한이 합산되어 direct dial 차단이 약화될 수 있다. ADR-0071의 실제 CNI 음성 probe를 완료 조건으로 사용한다.
