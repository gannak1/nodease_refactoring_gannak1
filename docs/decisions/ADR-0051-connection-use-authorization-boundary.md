...
3. Knowledge DB source 설정 저장 전과 외부 DB dial 직전에 같은 resolver로 검증한다. 실제 사용 조회는 현재 schema에서 가능한 TOCTOU 방어를 위해 Connection row를 잠그며, queue 대기 중 삭제 또는 owner 변경은 adapter 호출 전에 fail-closed한다.
...
- 외부 DB I/O 동안 Connection row lock을 보유하므로 같은 Connection의 수정·삭제가 대기할 수 있다.
- DB schema와 migration은 변경하지 않는다.

## 잔여 위험과 후속 검토

- 긴 외부 DB 호출 동안 row lock 대기가 운영상 문제가 되면 immutable credential revision 또는 bounded use lease를 별도 ADR로 검토한다.
