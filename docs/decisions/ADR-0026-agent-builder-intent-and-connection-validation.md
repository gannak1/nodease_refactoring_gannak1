...
Status: Accepted
Related ADRs: [ADR-0019](ADR-0019-agent-builder-preview-apply-save-boundary.md), [ADR-0024](ADR-0024-agent-builder-node-capability-catalog.md), [ADR-0025](ADR-0025-agent-builder-intent-model-selection.md)

## Context

Agent Builder는 permission-aware LLM structured output으로 자연어 의미를 추출하지만, schema가 유효하다는 사실만으로 요청 유형, 수정 대상, 배치 방향이 서로 일관된 것은 아니다. 또한 frontend handle과 graph validator가 차단하는 entry/terminal/Condition 연결을 backend가 같은 정책으로 재검증하지 않으면 preview 또는 apply/save에서 잘못된 graph가 저장될 수 있다.
...
Workflow Node Capability Catalog version 2가 다음 두 계약을 함께 소유한다.
...
Schema-valid 응답이 semantic invariant를 위반하면 extractor는 safe validation code만 포함해 정확히 한 번 repair 요청을 수행한다. 두 번째 결과도 유효하지 않거나 provider/JSON/schema 오류가 발생하면 `INTENT_EXTRACTION_FAILED`로 종료한다. Raw provider payload, credential, URL, path, node ID, edge ID는 repair prompt, 응답, draft, trace, audit에 저장하지 않는다. 정규식 기반 graph 생성으로 fallback하지 않는다.

Backend는 catalog의 connection policy로 complete candidate graph를 preview와 apply/save에서 모두 검증한다. 최소 차단 코드는 다음과 같다.
...
- Preview와 apply/save에서 같은 invalid graph가 같은 이유로 차단되며 저장되지 않는다.
