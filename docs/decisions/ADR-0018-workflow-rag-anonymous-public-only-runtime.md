...
현재 MVP에는 이 구조가 필요 이상으로 무겁다. Interactive 실행은 로그인 사용자를 execution subject로 전달할 수 있지만 public app, webhook, schedule, API secret 실행에는 아직 service account/operator 지정과 deployment preflight가 없다. `execution_subject`가 없다는 이유만으로 모든 RAG를 실패시키면 공개해도 되는 public documentation search까지 막게 된다.
...
- 이 ADR은 private RAG를 위한 deployment preflight UX/API를 승인하지 않는다.
