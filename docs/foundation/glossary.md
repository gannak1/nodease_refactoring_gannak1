...
| OrganizationMembership | User가 Organization에 직접 소속되어 있다는 1차 관계. MVP 2-0 목표 상태에서 `organization_memberships` table로 추가한다. |
| Team | Organization 안의 권한 부여 단위. 기존 `teams` table을 사용한다. |
| TeamMembership | User가 Organization 안의 Team에 배정되어 있다는 관계. 현재 코드는 `team_memberships`가 organization 소속까지 간접 표현하지만, MVP 2-0 이후에는 team 배정 관계로 축소한다. |
