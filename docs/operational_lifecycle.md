...
5. **FK 모순을 migration 전에 해소한다.** 현재 `llm_usage_logs.credential_id/model_id`는 `NOT NULL`인데 FK는 `ON DELETE SET NULL`이다. Soft revoke에서는 발현되지 않지만 hard purge/catalog delete 전에 nullable FK와 snapshot/tombstone 중 하나를 명시적으로 채택해야 한다.
