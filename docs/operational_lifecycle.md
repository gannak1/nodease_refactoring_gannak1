...
| Agent Builder durable intent plan L2 retention | `agent_builder_intent_plan_cache_records`의 encrypted cache-safe plan과 immutable `expires_at` | 별도 purge job row는 없으며 L2 record 자체가 lifecycle anchor다 | Gateway repository가 cache-eligible plan과 30-day expiry를 짧은 독립 transaction으로 commit한다. Shared Beat는 5분마다 task message만 `log` queue에 발행하며 cache audit/replay record를 만들지 않는다 | Log System task가 재시도 가능하게 최대 5개의 1,000-row batch를 처리한다. 각 batch는 candidate selection과 delete 모두에서 expiry를 재검사해 stale worker가 갱신 row를 삭제하지 못하게 하며, partial batch면 종료한다 | Pending Merge (JEO-7) |
...
| `agent_builder_intent_plan_cache_records` | 생성 DB 시각부터 30일간 encrypted cache-safe plan을 보존한다. read와 unexpired same-key 재저장은 expiry를 연장하지 않으며 5분 Beat가 bounded hard-delete한다 | `expires_at <= cutoff`, delete 직전 동일 expiry 재검사, organization FK cascade와 downgrade guard. L2는 legal hold·request replay·audit source가 아니며 cache payload를 별도 보존하지 않는다 | Pending Merge (JEO-7) |
