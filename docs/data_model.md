...
Agent Builder message history와 request `response_payload`는 사용자 경험 복구 목적의 기존 bounded TTL을 따른다. Parameter 값과 full typed operations는 request/session payload에 복제하지 않고 workflow graph와 일회성 API response가 각각 source of truth다. MBA-228의 단일 기능 PR은 nullable `AgentBuilderSession.protocol_version` additive migration과 null/direct protocol mixed read를 함께 제공한다. 신규 direct-edit session은 `direct_edit_v1`을 기록하고 기존 null row는 legacy로 해석해 backfill이나 자동 변환 없이 `stale_protocol`로 복구한다. Frontend와 Gateway의 무중단 coordinated rollout, 배포 gate와 image artifact 분리는 별도 배포 설계의 범위다. Request `response_payload`에는 request 생성 전에 정규화해 고정한 `mode_contract_version`과 `generation_mode_source`, canonical requested/effective `generation_mode`, mode transition 상태, `catalog_version=3`, operations 없는 safe operation envelope와 task safe state/version을 기록한다. 명시적 control과 `legacy-v1` default의 canonical mode는 request 생성 시 저장한다. `canonical-v2` default는 planning 동안 requested/effective mode를 미확정으로 두고 schema-valid planner 결과에서 intent mode 또는 guided fallback을 request lock 안에서 정확히 한 번 저장한다. Contract 값이 없는 기존 request는 `legacy-v1`로 읽고 JSON row를 backfill하지 않는다. Canonical mode는 `guided_generate|quick_generate|structure_only`이고 기존 `configure_and_generate`는 `guided_generate`로 읽기 정규화한다. `legacy-v1` request는 자연어 quick 의도를 활성화하지 않으며 direct quick 입력은 row 생성 전에 거부한다. Quick-to-guided 전환은 실제 parameter 값을 보존하지 않고 값 독립 plan과 재입력 대상 `step_id`/`parameter_key`만 저장한다. 전환 뒤 해당 task는 `resolution_source=null`, `reconfirmation_required=true`로 열고 실제 값은 typed decision과 workflow graph 저장 전까지 복구하거나 추측하지 않는다. Catalog version이 없거나 `2`인 미적용 envelope는 legacy stale로 분류하고, `3`인 envelope만 current catalog 검증으로 진행한다. GraphMutation 발급 전에 candidate를 검증해 `expected_result_graph_hash`를 기록하며 CAS save는 request graph hash와 이를 비교한다. CAS 저장 전 full operations 응답 유실은 request 재생성 또는 parameter 재입력으로 닫고, 저장 뒤 acknowledgement 유실만 persisted graph와 expected/saved hash로 복구한다. `generation_mode`와 mutation kind는 독립이며 새 workflow는 `initial_graph`, 기존 workflow 구조 변경은 `graph_edit`, 전체 교체는 `replace_workflow`를 사용한다. Parameter decision과 `after_graph` Knowledge binding 완료 상태는 canonical workflow graph hash와 `updated_at` acknowledgement 이후에만 전진한다. 최초 구조 mutation은 Agent Builder 시작 전 graph와 final graph의 history boundary를 만들고 각 parameter/Knowledge mutation은 별도 history entry 없이 boundary final hash/`updated_at`만 갱신한다. 완료 첫 Undo는 persisted task 상태를 바꾸지 않고 `completed|skipped|deferred` 중 최대 stable order의 재편집 가능 parameter UI를 client presentation에서 표시하며 다음 Undo는 시작 전 graph CAS 복구와 모든 task/Knowledge resolution cancel이다. Optional skip은 graph를 바꾸지 않고 task를 `skipped`로 보존한다. Redo history는 client memory에만 유지하고 reload 뒤 복구하지 않는다. Reload 전 Redo는 final graph만 CAS 저장하며 canceled Agent Builder 흐름을 자동 복원하지 않고, 그 뒤 다시 Undo하면 parameter 재진입 없이 같은 boundary를 즉시 revert한다. Task 상태와 mode transition은 parent request row lock, client operation id와 expected version으로 직렬화한다. 모든 비종료 RequestStatus의 contract-neutral cancel도 같은 row lock에서 request version을 전진시켜 늦은 planner/transition/task commit을 차단하고 남은 task/Knowledge resolution을 닫는다. Persisted graph와 완료 값은 cancel로 되돌리지 않는다. Quick-completion proposal이 `pending`에서 `acknowledged|canceled`로 바뀔 때 request와 proposal version을 같은 row lock에서 각각 증가시키고 둘 다 응답에 반환한다. 같은 operation 재시도는 기존 결과를 반환하고 competing decision은 conflict로 닫아 GraphMutation과 next task 중복 생성을 막는다. Workflow CAS save는 기존 `Workflow.updated_at`과 canonical persisted graph hash를 사용하며 workflow version column을 추가하지 않는다. CAS save는 final candidate graph에서 Catalog resource/credential reference, Knowledge/Collection binding과 WorkflowNode target을 서버가 추출하고 같은 transaction의 authoritative row·permission resolver로 모두 재검증한다. Client reference inventory나 발급 시점 allow 결과는 저장 권위가 아니다. Node `configuration_state`는 Catalog required configuration 전체에서 서버가 생성, parameter 변경, skip, Undo, recovery와 preflight마다 재계산한다. Graph save/revert/redo와 기존 `add_action_audit` insert는 같은 SQLAlchemy session/transaction에서 확정하며 신규 audit outbox나 worker를 추가하지 않는다. `response_payload`는 plain JSONB column이므로 진행 상태를 변경할 때 내부 dict만 in-place mutation하지 않고 변경된 전체 dict를 다시 대입한다. Agent Builder audit metadata는 graph 저장과 parameter task 진행을 추적하기 위한 값만 담으며 workflow 실행, Knowledge Base retrieval, Slack 전송, credential 사용/변경 또는 외부 시스템 변경의 성공을 의미하지 않는다.

Reference authorization은 새 table이나 client inventory를 만들지 않고 Catalog와 code-owned reference policy registry를 사용한다. Final graph의 managed reference는 authoritative resolver로 재검증한다. Resolver 미구현 Slack/GitHub `legacy_editor_connection`은 persisted base graph의 canonical field 값과 connection-relevant node data를 비교해 동일한 경우만 carry-forward하며 이 분류나 credential 원문을 `response_payload`에 복제하지 않는다. Unknown 또는 신규·변경 legacy field는 저장하지 않는다.

Session-scoped active-request cancel의 client operation id와 canceled request id/status는 기존 request safe operation metadata에 멱등 결과로 저장할 수 있다. 같은 session에는 비종료 RequestStatus row를 하나만 허용하며 request 생성과 session cancel이 session row lock으로 직렬화된다. Quick-completion acknowledgement는 완료되는 각 ParameterTask의 version을 정확히 한 번 증가시키고 task id/version/status 목록을 persisted idempotency result에 보존한다. 실제 provider attempt usage는 별도 usage source of truth가 소유하며 request cancel 뒤에도 이미 발생한 billing fact를 완료할 수 있지만 request payload에 token/cost를 복제하지 않는다.

RequestStatus와 safe operation envelope의 GraphMutationStatus는 별도 상태다. `graph_mutation_ready` request가 `pending_apply` GraphMutation을 포함할 수 있으며 `pending_apply`를 RequestStatus로 저장하지 않는다. 같은 operation 재시도에서 기존 결과를 반환한다는 계약은 DB에 보존된 safe 상태·validation 결과에만 적용한다. CDS 저장 전 full operations 응답을 잃은 mutation 발급은 operations를 복원하지 않고 `operation_payload_unavailable`로 닫으며 request 재생성 또는 현재 task/version의 새 operation id 재입력을 요구한다. Raw `X-Agent-Builder-Mode-Contract` header는 저장하지 않지만 새 request가 선택한 정규화 `mode_contract_version`은 활성 request 복구와 rollback 안전성을 위해 canonical request metadata로 고정한다.
