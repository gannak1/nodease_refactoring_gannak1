...
| `knowledge_collection_sync_job_items` | job이 snapshot한 내부 DB document target과 child 실행 상태 | organization/job/Collection/KB/document, deterministic position, pending/running/succeeded/failed/skipped, attempt/retryability/safe reason을 저장한다. `(job_id, document_id)`와 `(job_id, position)`을 unique하게 유지하고 API/audit/trace에는 child identity를 투영하지 않는다. |
