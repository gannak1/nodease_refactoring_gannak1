...
현재 코드의 SQLAlchemy `__tablename__` 기준 활성 테이블은 105개다. 아래 목록은 공통 model registry와 Alembic head `b19d0e1f2a34`를 대조한 inventory다. `legacy_llm_provider`, `legacy_llm_credentials`는 migration `e4956fcd7e2b`에서 DROP됐고 주석 처리된 호환 모델이므로 개수와 목록에서 제외한다. 테이블 추가·삭제 시 수동 개수만 바꾸지 말고 이 inventory와 해당 도메인 설명을 함께 갱신한다.
...
| Agent Builder | `agent_builder_sessions`, `agent_builder_requests`, `agent_builder_drafts`, `agent_builder_intent_plan_cache_records`, `agent_builder_intent_plan_semantic_cache_entries` |
...
- 위 ER diagram은 핵심 관계만 표시하며 105개 전체 table inventory를 반복하지 않는다.
...
다음 모델은 Accepted ADR-0045/0046의 MBA-228 Agent Builder 저장 계약과 ADR-0054의 생성 모드 계약이다. ADR-0019는 Superseded Preview 기록이며 활성 fallback의 근거로 사용하지 않는다. Workflow Editor 안에서 server-issued chat session, pending request, direct GraphMutation, CAS workflow draft 저장과 parameter task 상태를 복구하고 audit한다. ADR-0075의 `agent_builder_intent_plan_cache_records`만 cache-safe plan을 위한 additive durable L2 table이며, GraphMutation·request replay·audit 저장소가 아니다. GraphMutation을 local editor에 적용한 것만으로 완료로 보지 않으며, canonical persisted graph hash와 workflow `updated_at` acknowledgement 이후에만 parameter task를 활성화한다.
...
| `AgentBuilderRequest` | request id, session id, request status, safe structured request summary, allowlisted `intent_cache_outcome` (`disabled|hit|miss|bypass|error`), `response_payload` 안의 request-scoped `mode_contract_version`과 monotonic `request_version`, canonical/requested/effective `generation_mode`와 `generation_mode_source`, mode transition 상태와 safe reason code, quick-to-guided 전환 시 값 독립 plan 및 재입력 대상 `step_id`/`parameter_key`, quick-completion proposal의 `pending|acknowledged|canceled|stale` 상태·monotonic `proposal_version`·fenced target task id/version/fingerprint·canonical graph hash/workflow `updated_at`, session/request operation id/kind/status와 persisted idempotency result, `catalog_version`, base graph hash, 발급 시 계산한 expected result graph hash, expected workflow `updated_at`, affected node ids, completion context, canonical save graph hash/`updated_at`, acknowledgement 또는 reverted 상태로 구성된 safe operation envelope, 모든 configurable parameter task의 node/parameter key, task version, defer policy, `reconfirmation_required`, pending/active/completed/deferred/skipped/invalid/canceled 상태와 자동 확정 resolution source, created_at/completed_at/canceled_at/expires_at | Raw negotiation header, L1/L2 source, lookup token, cache envelope/plan, full typed GraphMutation operations, proposal의 실제 parameter 값·node data·graph fragment, 값에 의존하는 미저장 graph fragment, raw graph snapshot, raw provider response, hidden KB/source detail, secret-like user input 원문, credential config 저장 금지 |
| `AgentBuilderIntentPlanCacheRecord` | organization FK with cascade, versioned repository lookup token, encrypted strict `CachedIntentPlanV1` envelope, envelope MAC, encryption key version/algorithm, created_at와 immutable 30-day expires_at; organization+lookup-version+token unique index와 expiry index | raw request, Redis key/digest, graph, node/edge/request/operation UUID, credential reference/config, parameter value, permission decision, raw provider payload, current authorization/Knowledge selection handle 저장 금지 |
| `AgentBuilderIntentPlanSemanticCacheEntry` | organization/user scope, generation mode, semantic projection·embedding profile/model·planner/catalog/normalizer/rehydration contract version, safe query embedding, parent L2 FK, child created_at와 parent를 anchor로 한 expires_at; composite tenant FK, scope/version/expiry index와 cascade | raw request/query/projection text, graph, node/edge/request/operation UUID, credential/Knowledge/Connection reference, parameter value, permission decision, vector score, plan/envelope copy와 provider payload 저장 금지 |
...
Agent Builder message history와 request `response_payload`는 사용자 경험 복구 목적의 기존 bounded TTL을 따른다. Parameter 값과 full typed operations는 request/session payload에 복제하지 않고 workflow graph와 일회성 API response가 각각 source of truth다. L2 row는 read로 expiry를 연장하지 않으며 bounded background purge가 만료 뒤 hard-delete한다. organization 삭제는 FK cascade로 L2 row를 제거하고 migration downgrade는 L2 row가 남아 있으면 거부한다. MBA-228은 nullable `AgentBuilderSession.protocol_version` additive migration과 null/direct protocol mixed read를 함께 제공한다. 신규 direct-edit session은 `direct_edit_v1`을 기록하고 기존 null row는 backfill 없이 `stale_protocol`로 복구한다. Frontend/Gateway 무중단 rollout과 image artifact 분리는 별도 배포 설계가 소유한다.

#### JEO-8 private dense semantic index

ADR-0076의 `AgentBuilderIntentPlanSemanticCacheEntry`는 migration `b19d0e1f2a34`에 포함된 additive model이다. Target-independent `new_workflow`에 한해 organization/user scope, generation mode, `semantic_query_projection_version`, embedding profile/model version, planner/catalog/normalizer 및 required rehydration contract version, redaction-safe query embedding, parent `AgentBuilderIntentPlanCacheRecord` FK, created/expires timestamp만 저장한다. `selected_target`, modify/replace request와 transient `SemanticQueryProjectionV1` text는 저장 대상이 아니다. Raw request/query text, graph, node/edge UUID, credential/Knowledge/Connection ID, explicit parameter value, permission decision, vector score, L2 plan/envelope copy와 provider payload도 저장하지 않는다.

Parent L2 plan row는 plan authority와 child의 intended immutable expiry를 유지한다. Migration은 `organization_id + intent_plan_record_id` composite FK로 parent organization을 맞추고 organization/user/parent `ON DELETE CASCADE`, scope/version/expiry filter index, parent/profile/projection/contract-version unique constraint와 semantic-row downgrade guard를 제공한다. Child parent ID, expiry와 conflict policy는 같은 짧은 independent L2 transaction에서 unique row를 lock하고 DB가 확인한 L2-only `IntentPlanL2StoredReceipt(inserted|unexpired_conflict|expired_replacement)`만 사용한다. Missing-row concurrent insert loser만 같은 transaction에서 한 번 locked read를 허용하며 post-commit re-read, 추정 ID와 무제한 retry는 금지한다. Append SQL은 현재 `IntentPlanningContext.organization_id`와 receipt parent ID/expiry equality를 검사한다. Insert와 unexpired conflict는 missing child를 만들고 existing conflict는 `DO NOTHING`이다. Expired replacement는 retention이 child를 먼저 삭제했으면 insert하고, existing child expiry가 receipt보다 과거일 때만 embedding/created/expiry를 원자 교체한다. Parent mismatch, late receipt와 same/newer child는 zero-write다. Usable child expiry는 parent immutable 30-day expiry와 같아야 하고 read는 두 expiry의 unexpired/equality를 모두 검사한다. Existing L2 retention task는 expired child를 savepoint/별도 transaction에서 bounded purge하고 delete-time expiry를 다시 검사한 뒤 expired parent를 삭제하며 parent cascade가 남은 child를 제거한다. Readiness는 해당 child 단계만 rollback하고 다음 Beat에 같은 worker가 재시도한다. Child/parent 중 어느 한 count라도 per-kind limit이면 기존 bounded loop를 계속한다. 따라서 in-place parent renewal 뒤 child replacement가 실패하면 mismatch/expired old child는 즉시 non-retrievable하고 다음 schema-ready retention 실행에서 삭제된다. L2 row가 남으면 JEO-7 downgrade guard가 계속 적용된다.
