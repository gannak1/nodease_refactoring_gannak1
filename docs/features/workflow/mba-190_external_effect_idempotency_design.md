...
이 문서는 합의된 MBA-190 공통 계약을 설명하는 Draft다. Migration, runtime, 현재 Generic HTTP/Slack/GitHub 연결, 재진입 복구, readiness와 관련 자동화·실제 PostgreSQL 테스트는 통과했다. 전체 자동 검토와 커밋 확정 전에는 `Verified Against`를 기록하거나 이슈를 Done으로 처리하지 않는다.
