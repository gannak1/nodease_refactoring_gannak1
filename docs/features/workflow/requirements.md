...
- 빌더로서, 워크플로우 테스트가 모두 끝난 뒤 전체 실행 시간과 전체 비용을 최종 요약으로 확인하고 싶다.
...
- 워크플로우 테스트가 완료되면 테스트 실행 사이드바의 마지막 영역에 전체 실행 시간, 전체 비용, 전체 토큰 사용량을 최종 요약으로 표시해야 한다.
...
- 소요 시간은 프론트가 `node_start`와 `node_finish` 이벤트 수신 시각 기준으로 계산한다. 백엔드의 영구 실행 로그와 완전히 같은 값이라고 보장하지 않는다.
...
- 패널 폭 계산은 렌더된 편집 영역의 실제 가로 폭을 기준으로 한다.
...
- 백엔드 스트리밍 이벤트에 node-level token/cost/latency를 표준 필드로 포함할지 여부.
- A/B 비교 실행 결과를 테스트 실행 사이드바 안에서 확장할지, 별도 비교 패널로 분리할지 여부.

### 2. 노드 조작 편의성

- 패널 비율을 사용자별 preference로 영구 저장할지 여부.

### 3. 워크플로우 조작 편의성

- branching/condition/loop 같은 특수 노드 삭제 시 자동 재연결을 어디까지 허용할지 여부.

### 4. 노드 실행 기록 패널 추가

- 기존 실행 로그 API만으로 현재 node_id 기록이 있는 run 목록을 효율적으로 조회할 수 있는지 여부.
- node_id 기반 실행 로그 검색 API를 별도로 추가할지 여부.
- input/output full payload 표시 범위와 redaction 기준을 어디까지 프론트에서 보완할지 여부.
