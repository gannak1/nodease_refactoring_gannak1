...
- Generic HTTP/Slack/GitHub comment 성공과 실패에서 workflow downstream output은 기존 값을 유지하지만 Workflow Engine log command와 Log System 저장 row/payload에는 safe provider summary만 남는다. Opaque body/header/input, provider exception 원문과 encrypted raw payload는 남지 않는다.
