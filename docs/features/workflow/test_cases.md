...
- Schedule 활성화·재활성화와 dispatch preflight는 canonical deployment creator의 External Action Credential `use`를 검사하고 요청 actor나 queue 값으로 대체하지 않는다. Workflow Engine Slack/GitHub node는 같은 user형 credential principal로 다시 검사하며, principal이 없거나 malformed/non-user이면 task publish 또는 provider I/O 전에 fail-closed한다. 이 principal을 Knowledge execution subject나 user audit actor로 승격하지 않는다.
