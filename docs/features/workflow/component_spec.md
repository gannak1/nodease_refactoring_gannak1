...
- `TestSidebar`
  ...
  - 워크플로우 테스트가 완료되면 마지막 영역에 전체 실행 시간, 전체 비용, 전체 토큰 사용량을 최종 요약으로 표시한다.
- `BottomPanel`
  - 기본 캔버스 조작 도구만 유지한다.
  - 테스트 실행 요약을 표시하지 않는다.
...
- Resizable three-panel layout
  - 기본 비율은 세 패널이 균형 있게 보이는 값으로 시작한다.
  ...
  - 패널 폭 계산은 고정된 전체 px 값이 아니라 실제 렌더된 편집 영역의 가로 폭을 기준으로 한다.
...
- 워크플로우 테스트가 완료되면 테스트 실행 사이드바의 마지막 영역에 전체 실행 시간, 전체 비용, 전체 토큰 사용량을 표시한다.
