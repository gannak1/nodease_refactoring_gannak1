...
- `TestSidebar`
  ...
  - 전체 실행 비교는 성공·실패 같은 상태를 A/B 배지로 분리하고, 비용·실행 시간·전체 토큰은 같은 행의 회색 A baseline 막대와 파란색 B candidate 막대로 표시한다. 변화율은 비용·실행 시간의 감소만 개선 색상으로 표시하며, 토큰 변화는 품질 향상 또는 저하를 단정하지 않는다.
