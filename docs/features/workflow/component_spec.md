...
- Provider-neutral Helm의 Worker egress NetworkPolicy는 cluster DNS, DB/Redis/Sandbox service, Mail IMAP port와 internal Squid `3129`만 허용한다. Public HTTP/HTTPS direct route는 두지 않으며 Generic HTTP의 public 80과 HTTPS CONNECT 443은 application guard를 통과한 뒤 Squid listener에서만 허용한다. 외부 dependency CIDR은 해당 service port에만 한정한다. Cluster는 policy를 실제 집행하는 CNI를 사용해야 하며, additive allow policy와 node-local/`hostNetwork` 예외가 없는지는 배포 전 positive/negative probe로 확인한다.
