...
- `TestSidebar`
  ...
  - 기준 실행 목록의 `상세` 조회가 durable run 저장 지연으로 `404`를 반환하면 한 번 짧게 다시 조회한다. 재시도 후에도 찾지 못하거나 다른 오류가 발생하면 원인을 구분해 안내하고 사용자가 같은 행에서 `다시 불러오기`를 실행할 수 있어야 한다.
...
- 실행 중 상태는 각 노드 카드의 상태 배지로 표시하며, 노드 목록 위에 같은 의미의 전역 `테스트 실행 중` 제목을 중복 표시하지 않는다.
