...
- `TestSidebar`
  ...
  - 같은 workflow의 보고 화면을 거쳐 돌아오거나 새로고침하면, URL의 실행 식별자로 권한 확인된 workflow run 상세를 다시 읽어 마지막 실행의 결과·노드 카드·선택 상세를 복원한다.
  ...
  - 사용자가 `기준으로 고정`을 누른 실행은 비교 모드를 종료하거나 `기준 변경`을 누르기 전까지 유지한다. `다시 테스트하기`와 현재 workflow 재실행은 기준 실행을 지우지 않는다.
