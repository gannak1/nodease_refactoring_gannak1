...
Status: Draft
Verified Against: `feature/mba-275 @ 48eb5d5e`
...
- `TestSidebar`
  ...
  - 현재 실행이 `running`인 동안에는 비교 API를 실패로 처리하지 않고 `현재 실행이 완료되면 비교 결과를 준비합니다` 안내를 표시한다. terminal 상태로 전환된 뒤에만 현재 실행 상세를 조회하며, node run 기록이 아직 반영되지 않은 `404`/준비 중 상태는 제한된 backoff로 다시 조회한다.
