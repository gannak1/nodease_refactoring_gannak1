...
- 스트리밍 실행의 `error` 이벤트는 최소 고정 code인 `external_effect.result_unavailable`, `external_effect.identity_conflict`, `external_effect.outcome_unknown` 중 해당 값, digest를 만들 수 없는 준비 실패의 `external_effect.prepare_failed`, 또는 `failed_before_effect + stop`의 기존 allowlisted provider/configuration code를 전달할 수 있다. 모든 경우 safe `message`, terminal 오류의 `retryable=false`와 해당 node를 식별할 수 있을 때 `node_id`를 함께 포함한다.
