...
챗봇 배포는 기존 배포/공개 실행 엔드포인트의 계약을 확장한다. 현재 generic 인증 배포 실행 endpoint는 로그인 사용자를 execution subject로 전달할 수 있다. 그러나 Target private-RAG 내부 Chatbot은 별도 내부 Chatbot 이용 권한, deployment runtime policy와 session namespace를 갖는 후속 기능이며 이 문서의 current generic endpoint와 동일한 완성 상태로 간주하지 않는다.
...
| GET | `/api/v1/deployments/{deployment_id}/run-info` | Current generic 실행 화면용 safe 배포 정보 조회. Target internal Chatbot session endpoint가 아님 | 로그인 + workflow `execute` 권한 |
| POST | `/api/v1/deployments/{deployment_id}/run` | Current generic 활성 배포 snapshot을 로그인 사용자 권한 주체로 실행 | 로그인 + workflow `execute` 권한 |
...
- `authenticated_internal_chatbot`: 별도 내부 Chatbot 이용 권한, cookie authentication/CSRF, exact Origin, current user KB permission과 독립 session namespace를 사용한다. 별도 기능 구현 전 미지원이다.
...
- Current generic 인증 실행(`/deployments/{deployment_id}/run`)은 workflow `execute` 권한을 요구하며 로그인 사용자를 RAG execution subject로 전달한다. 이는 Target 내부 Chatbot 이용 권한이나 Conversation Session surface를 대신하지 않는다.
- Target authenticated internal Chatbot은 별도 내부 Chatbot 이용 권한과 current KB permission/source ACL을 모두 요구한다. Public grant나 workflow `execute`만으로 허용하지 않는다.
