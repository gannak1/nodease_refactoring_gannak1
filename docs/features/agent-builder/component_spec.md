...
모델 또는 credential 삭제 뒤에는 usage row의 연결 ID만 NULL이 될 수 있다. 당시 token/cost와 user/organization/workflow 귀속은 보존한다. Agent Builder 전용 endpoint/대시보드와 Workflow Engine usage 재설계는 이 component 범위가 아니며, 기존 관리 화면과 내 모듈 workflow별 비용 영역에 additive 비용 구분만 제공한다. 내 모듈의 page-level 비용·추세·위험 요약은 ADR-0060에 따라 표시하지 않는다.
