...
- Gmail 답장 자동화 preview는 durable Mail 검색, Gmail Draft, Mail Acknowledge node를 구분해 표시한다. Draft node에는 발송 action이 없으며 Acknowledge node는 processing/effect output 연결만 표시한다.
