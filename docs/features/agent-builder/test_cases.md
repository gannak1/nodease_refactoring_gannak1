...
- Provider 실패 또는 usage 누락은 pending 행을 삭제하고, 완료되지 않은 pending 행은 관리 비용·token·호출 수·Top Model뿐 아니라 workflow 예산과 내 모듈 workflow별 월 예상 비용 집계에서도 제외하는지 검증한다.
...
- 기존 user top-model의 활성·비활성 model 행, Admin organization/workflow, workflow budget과 내 모듈 workflow별 월 예상 비용 집계가 Agent Builder usage를 포함한다. Model 삭제 뒤 해당 행은 Top Model에서 제외되지만 나머지 비용 집계에서는 보존된다.
- Admin과 내 모듈 workflow별 응답은 기존 총비용을 유지하고 workflow 실행 비용과 Agent Builder 비용을 구분하며 두 값의 합이 총비용과 같은지 검증한다. 내 모듈은 page-level 비용·추세·위험 요약을 렌더링하거나 `cost-summary`를 호출하지 않는다.
...
- 모델·credential 삭제 뒤 연결 ID만 NULL이 되고 token/cost와 organization/workflow/budget/내 모듈 workflow별 예상 비용 집계가 유지되는지 검증한다.
