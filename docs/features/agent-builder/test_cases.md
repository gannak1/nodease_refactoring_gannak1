...
- Slack API mode의 Bot Token, channel과 payload, Webhook mode의 Webhook URL과 payload를 Agent Builder task로 제공한다. Bot Token/Webhook URL은 mode별 필수 masked input이며 건너뛸 수 없고, channel의 명시적 defer와 미설정 인증값은 unresolved 저장 뒤 test/run/deploy를 차단한다.
- GitHub API Token은 `get_pr|comment_pr` 모두 runtime/preflight 필수 masked Agent Builder task다. Legacy session의 중복 token task는 current Catalog identity로 하나만 reconciliation한다.
