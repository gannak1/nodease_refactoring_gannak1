...
- Gmail 답장 초안 요청은 durable Mail 검색, unresolved Gmail Draft와 Mail Acknowledge node를 연결한다. Preview/apply-save 중 OAuth token refresh, Gmail draft 생성, 읽음 처리 또는 send 호출이 없어야 한다.
