...
- Slack/GitHub `secret` definition은 `agent_builder_task=false`로 Agent Builder task/card/API payload에 포함하지 않는다. Legacy/malformed secret task가 복구돼도 입력 control이나 save bridge를 표시하지 않고 raw `set`은 fingerprint 계산 전 `secret_forbidden`으로 거부한다.
- Mail/Gmail managed credential task는 Undo reentry와 구분되어 권한 검증된 후보만 표시하고, deferred presentation reentry는 기존 값과 상태를 안전하게 보여준다. Slack/GitHub managed credential task, 빈 picker와 raw secret control은 direct-edit response/UI에 포함하지 않으며 Catalog의 비밀이 아닌 일반 파라미터만 순차 확인한다.
...
17. direct-edit response와 legacy session 정규화 뒤 Slack/GitHub managed credential task, 빈 picker, credential defer와 secret task/control이 없고 Catalog의 비밀이 아닌 일반 파라미터만 남는지 확인한다.
...
- A Slack/GitHub direct-edit plan exposes every Catalog-declared non-secret Agent Builder parameter and omits token/URL secret fields marked `agent_builder_task=false`. It exposes no `credential_ref` task, credential candidate, empty picker, credential defer control, or raw secret input. Mail/Gmail still expose only permitted managed credential candidates.
- Slack mode와 parameter 설명은 Catalog의 한국어 문구를 유지한다. API mode는 channel 등 비밀이 아닌 parameter만, Incoming Webhook mode도 비밀이 아닌 parameter만 표시한다. Bot Token과 Webhook URL은 `agent_builder_task=false`이므로 Agent Builder task/card/API payload에서 제외하고, legacy session의 해당 secret task는 reconciliation에서 제거한다. Mode 변경은 반대 mode의 비밀이 아닌 기존 설정을 canonical node data에서 정리하고 현재 mode의 다음 유효한 task를 활성화한다. 인증값이 없으면 node는 unresolved로 남아 test/run/deploy preflight에서 차단된다. `message`는 `{{result}}`와 이전 node output 연결 방법을 설명한다. 값이 없는 Optional Blocks/Attachments의 빈 `적용`은 `skip`으로 완료되고, 기존 optional text/JSON/select 값을 편집 중 비운 `적용`은 `clear` GraphMutation과 CAS/acknowledgement 뒤 graph 값을 제거하고 task를 `skipped`로 만든다.
...
- 일반 ParameterDecision `set`의 secret-like 값과 detector 오류는 fail-closed하며 일반 control은 `400 invalid_decision`, 조작된 `input_type=secret` decision은 `400 secret_forbidden`으로 반환한다. Agent Builder는 secret task/control/save bridge를 발급하지 않으며 어느 경로에서도 graph/session/task/audit/trace/log/응답에 원문이 남지 않는다. 일반 Catalog validation issue는 HTTP 오류가 아니라 `status=invalid`, `reason=catalog_validation_failed` task 결과로 저장·반환된다.
...
- Resolver 미구현 기존 Slack/GitHub Editor connection이 base graph와 candidate에서 canonical field 값 및 connection-relevant node data가 같으면 다른 node의 Agent Builder mutation 저장을 허용한다. Agent Builder GraphMutation이 같은 legacy field를 추가·교체·삭제하거나 새 node로 복제하면 전체 save/audit를 rollback한다. Agent Builder UI나 일반 workflow save bridge로 해당 secret field를 추가·교체하지 않으며 test/run/deployment/runtime의 기존 연결 검증을 통과하지 않으면 외부 호출은 차단된다.
...
## Test preflight 및 secret 제외 회귀 검증

- Slack Bot Token, Slack Webhook URL과 GitHub API Token definition이 `agent_builder_task=false`이고 direct-edit task/card/API response에 포함되지 않는지 검증한다.
...
- Legacy session의 secret task는 Catalog reconciliation에서 제거되고, frontend에 조작된 task가 도착해도 password input, save bridge 또는 clear/skip action을 제공하지 않는지 검증한다.
...
## Secret 제외와 이전 task action 회귀

- Slack Bot Token, Slack Incoming Webhook URL과 GitHub API Token task가 direct-edit plan과 card에 표시되지 않는지 검증한다.
- Legacy/malformed secret task가 입력 control이나 save bridge를 열지 않고 조작된 decision이 `secret_forbidden`으로 닫히며 API 응답·task/session·audit·trace·log에 원문 또는 값 파생 fingerprint가 남지 않는지 검증한다.
...
- GitHub API Token은 `get_pr|comment_pr` 모두 runtime/preflight 필수지만 Agent Builder task에는 포함하지 않는다. Legacy session의 token task는 reconciliation에서 제거한다.
