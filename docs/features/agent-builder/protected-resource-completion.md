...
Working Tree Base: feature/mba-277 @ 9d48c94f (uncommitted changes; commit-pinned verification unavailable)
...
| 관리 UI·catalog·picker | 완료 | ADR-0045 typed ParameterTask와 ADR-0061 hierarchy-only Knowledge card | `KnowledgeSelectionControl`, `ParameterInputRenderer`, `NodeParameterCard`, `AgentBuilderPanel` | Current working tree frontend 8 files / 258 passed; TypeScript와 lint 0 errors | 인증된 실제 browser smoke는 미실행 |
| 저장 schema·GraphMutation·redaction | 완료 | ADR-0045/0046 safe envelope, raw secret의 ParameterDecision/GraphMutation/session 비복제 | `workflow_node_catalog.py`, `AgentBuilderPanel.tsx`, `ParameterInputRenderer.tsx` | `test_agent_builder_parameter_tasks.py`, focused frontend tests | Slack/GitHub secret task는 safe metadata만 발급하고 masked input의 새 값은 기존 editor draft save bridge로 전달 |
...
| Slack/GitHub secret | Catalog `secret` + mode별 `visible_when|required_when` | 기존 raw 값은 hydrate하지 않고 새 입력만 editor mapping으로 저장 | masked password input, Node Detail 강제 이동 없음 | Agent Builder decision/GraphMutation/session에는 raw value 없음; canonical draft save 뒤 session 재조회 | 미설정 node를 preflight/runtime에서 fail-closed | frontend masked-input/save-bridge tests, planner inclusion과 backend raw-decision `secret_forbidden` tests |
...
| Client static | `npm run typecheck`; `npm run lint` | typecheck passed; full lint 0 errors / 248 existing warnings; production build는 실행 중 dev server와 `.next`를 공유하지 않기 위해 미실행 |
