...
Panel header는 [ADR-0040](../../decisions/ADR-0040-agent-builder-unified-model-recommendation.md)에 따라 Agent Builder 내부 intent planner가 사용할 model을 표시하고 변경할 수 있어야 한다. Provider group은 `openai`, `anthropic`, `google`, `llamaparse` 순서로 표시하며, active organization에 속하고 `LLMCredential.is_valid=true`, `LLMModel.is_active=true`, `LLMModel.type=chat`, model과 credential의 provider가 일치하고 `LLMRelCredentialModel.is_verified=true`, 사용자 credential `use` 권한을 모두 통과한 조합을 선택할 수 있다. 같은 model/credential 관계가 중복이면 가장 낮은 relation priority 하나만 후보로 사용한다. 정규화한 API model ID로 특수 목적이 확정된 모델은 제외하되 세대나 tier를 해석하지 못한 verified chat 모델은 provider의 해석 가능한 모델 뒤에 유지한다. LlamaParse는 chat model을 지원하지 않는 disabled group으로 표시한다. Header의 최초 기본 option은 AB-FR-009의 generated LLM node와 같은 통합 추천 정책으로 결정하지만, 사용자는 다른 option을 선택할 수 있고 Header 선택은 generated node model로 자동 복사되지 않는다.
...
Model option은 provider를 `openai`, `anthropic`, `google` 순서로 평가하고 provider 안에서 최신 세대를 먼저 표시한다. 같은 provider와 세대에서는 `general`, `mini`, `nano`, `pro` 순서로 정렬하고, 같은 tier에서는 suffix가 없는 기본형, 날짜 또는 명시 release snapshot, `preview`, `latest` 순서로 둔 뒤 verified relation priority, safe model/credential display name과 안정적인 식별자로 결정적 순서를 보장한다. Provider별 세대와 tier 해석은 작은 명시적 정책표를 사용한다. 날짜, release, `preview`, `latest` suffix는 세대/tier 계산에서 제외할 수 있지만 실제 model ID를 바꾸지 않는다. Sora, audio, transcribe, TTS, speech, Whisper, realtime, live, image, embedding, moderation, search, Codex, computer-use, robotics처럼 특수 목적이 확정된 모델은 전역 model catalog에는 유지하되 Agent Builder option과 자동 추천에서는 제외한다. 특수 목적 판정은 정규화한 model ID를 `-`, `_`, `.`, `/`, `:`로 나눈 완전한 token 또는 명시된 연속 token과 provider별 전체 model ID 일치 규칙만 사용하고 단순 부분 문자열 일치는 사용하지 않는다. 세대나 tier를 해석하지 못했지만 DB 자격 조건을 통과한 verified chat 모델은 임의 등급을 부여하지 않고 provider의 해석 가능한 모델 뒤에 안정적인 순서로 유지한다. Agent Builder는 provider별 고정 저비용 model map으로 선택값을 숨겨 대체하지 않는다.

이 순서는 intent planner Header와 AB-FR-009의 generated workflow LLM node가 공유하는 기본 추천 순서다. 동일한 권한 후보 집합에서 Header의 최초 기본 option과 generated LLM node의 기본 model은 같아야 한다. Header에서 사용자가 선택한 intent planner model은 요청별 planner 실행에만 사용하고 workflow node model로 복사하지 않는다.
...
지원 capability allowlist는 [ADR-0024](../../decisions/ADR-0024-agent-builder-node-capability-catalog.md)의 공통 Workflow Node Capability Catalog를 기준으로 한다. 구현된 node type 중 `agent_builder_supported=true`인 node만 포함하며 `loopNode`는 runtime 구현 여부와 별개로 현재 제품 가용성이 비활성 상태이므로 제외한다. Mail 답장 자동화에는 `mailNode`, `gmailDraftNode`, `mailAcknowledgeNode` capability를 사용한다. `githubNode`는 현재 PR 조회와 PR 댓글 등록 capability를 구분하며, 요청에 두 동작이 모두 필요하면 별도 node로 생성한다. PR 생성 operation은 자연어 의미로는 인식하지만 runtime, editor, catalog capability가 준비되기 전까지 draft node로 materialize하지 않는다.
...
Generated LLM node의 기본 `model_id`는 active organization에 속하고 `LLMCredential.is_valid=true`, `LLMModel.is_active=true`, `LLMModel.type=chat`, model과 credential의 provider가 일치하고 `LLMRelCredentialModel.is_verified=true`, 사용자 credential `use` 권한을 통과한 model 후보에서 추천한다. 같은 model/credential 관계가 중복이면 가장 낮은 relation priority 하나만 후보로 사용한다. 정규화한 API model ID로 특수 목적이 확정된 모델은 제외하고, 세대나 tier를 해석하지 못한 verified chat 모델은 해석 가능한 모델 뒤의 fallback 후보로 유지한다. Header와 같은 통합 추천 정책으로 provider를 `openai`, `anthropic`, `google` 순서로 평가하고 provider 안에서는 최신 세대, 같은 세대 `general`, `mini`, `nano`, `pro`, 같은 tier의 기본형, 날짜 또는 명시 release snapshot, `preview`, `latest` 순서로 첫 사용 가능 후보를 추천한다. MBA-240의 `gpt-5.5` 회귀 사례는 같은 5.5 세대의 general 우선과 기본형 우선 규칙으로 충족하며, 더 최신 세대가 있으면 특정 모델 고정 없이 최신 세대를 먼저 추천한다. Workflow graph에는 model id만 저장하며 추천에 사용된 credential id나 원문은 저장하지 않는다. 후보가 없으면 model id를 비우고 `configuration_state=unresolved`와 model 설정 필요 warning을 남기며, 고정 환경변수 model route 때문에 draft 생성을 실패시키지 않는다. 기존 workflow를 수정할 때는 기존 LLM node의 저장 model을 덮어쓰지 않고 새로 생성한 LLM node에만 현재 추천을 적용한다. Preview Mode에서는 model 설정을 읽기 전용으로 표시하고, 사용자는 적용 및 저장 후 기존 Workflow Editor에서 LLM node model을 다른 사용 가능한 값으로 변경해 저장할 수 있다.
...
Primary 전환, Deployment 생성, Deployment 활성/비활성 전환은 같은 App lifecycle row lock을 사용해 write command를 직렬화해야 한다. App row는 잠금 획득 뒤 SQLAlchemy identity map의 기존 객체를 재사용하지 않고 canonical DB 상태로 강제 갱신해야 한다. Snapshot을 생략한 Deployment 생성은 갱신된 primary의 server graph를 사용한다. 명시적 client snapshot 요청은 Gateway가 잠금 전에 관찰한 primary에 binding하고, 대기 중 primary가 바뀌면 `409 deployment.graph_snapshot_stale`로 fail-closed한다. 실행과 run-info 같은 read-only runtime 표면은 exclusive lifecycle lock을 사용해 동일 App 실행을 직렬화해서는 안 된다. 현재 Deployment schema에는 immutable source Workflow ID가 없으므로 과거 inactive Deployment의 정확한 provenance를 추론하지 않는다. 대신 App에 Workflow 이력이 둘 이상이면 inactive Deployment 재활성화를 fail-closed하고 현재 primary에서 새 Deployment를 만들도록 요구한다.

기존 primary에 활성 WorkflowBudget이 있으면 primary 전환은 예산 row나 당월 usage를 임의 복제·초기화하지 않고 `APP_WORKFLOW_BUDGET_CONFLICT`로 차단해야 한다. 활성 예산 확인과 예산 upsert는 같은 organization/Workflow advisory transaction scope를 사용해 검사와 동시 변경을 직렬화한다. 예산 upsert는 그보다 먼저 App lifecycle row를 잠그고 요청 Workflow가 여전히 current primary인지 재검증하며, 전환에 밀린 stale 요청은 `409 workflow.primary_changed`로 거부한다. 사용자 안내와 audit-safe block metadata에는 예산 금액이나 당월 비용 원문을 포함하지 않는다.

Primary 전환이 허용되면 기존 primary Workflow에 속한 같은 organization의 직접 사용자·팀 Workflow 권한을 새 Workflow로 승계하고, 적용 actor의 manager 권한을 보장한다. 승계 snapshot과 direct Workflow 권한 PUT·DELETE는 같은 organization/Workflow permission scope lock을 공유해, 먼저 commit된 revoke가 새 primary에서 되살아나지 않게 해야 한다. User direct 권한 mutation은 중앙 Access Management와 legacy API 모두 `access subject -> App lifecycle -> Workflow permission scope -> permission key` 순서로 잠그고, team direct mutation은 subject 단계 없이 `App lifecycle -> Workflow permission scope -> permission key` 순서를 사용한다. Access subject row lock은 subject 상태 mutation끼리는 직렬화하되 permission FK의 `KEY SHARE`와 호환되는 `FOR NO KEY UPDATE` 또는 동등한 강도를 사용해야 한다. 요청이 primary를 관찰한 뒤 전환에 밀렸으면 성공으로 처리하지 않고 `409 workflow.primary_changed`로 재시도를 요구한다. Organization member 제거는 access subject lock을 잡은 뒤 해당 사용자의 Workflow permission scope 집합을 안정될 때까지 재조회·잠금하고 bulk revoke하여, 대기 중 승계된 target grant까지 같은 transaction에서 제거해야 한다. 요청 시작부터 이미 non-primary였던 Workflow의 독립적인 권한 관리는 기존 계약을 유지한다. 새 Workflow 생성, 권한 승계, App primary 갱신, Agent Builder session 재연결, apply/save audit는 같은 transaction에서 완료되어야 한다. 차단 또는 실패 시 기존 App primary와 권한 상태를 유지한다.
