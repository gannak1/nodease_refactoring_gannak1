...
- `secret`은 기존 node graph가 직접 소유하는 민감 설정을 위한 Catalog 입력 유형이다. Direct-edit UI는 Agent Builder card 안에 masked password control을 제공한다. 값은 browser component memory에서만 보유하고 기존 값을 hydrate하거나 다시 표시하지 않는다. 사용자가 적용하면 Agent Builder ParameterTask `set` API가 아니라 기존 Workflow Editor secret node-data save adapter와 workflow CAS를 호출하며, GraphMutation 응답, Agent Builder API 응답, task/session, audit, trace 또는 log에 값을 포함하지 않는다.
- Agent Builder secret control로 canonical save가 성공한 뒤 client는 갱신된 graph hash와 `updated_at`을 반영하고 Agent Builder session을 다시 조회한다. Backend는 raw 값이나 값에서 파생한 fingerprint 없이 Catalog mapping 위치의 설정 존재 여부만으로 secret task를 `completed`로 조정한다. Optional secret을 명시적으로 지우면 Workflow Editor save/CAS가 canonical 값을 제거한 뒤 해당 task를 `skipped`로 닫고, required secret이 제거되면 같은 task를 `pending|active`로 다시 열어 완료 상태를 차단한다. 저장 실패 중에는 현재 component memory의 입력을 유지해 사용자가 재시도할 수 있게 하고, 성공·닫기·workflow/session 전환 시 즉시 폐기한다.
- Catalog v3 parameter의 `defer_policy`는 `forbidden` 또는 `allow_unresolved`이며 생략하면 `forbidden`이다.
- `credential_ref`를 포함한 configurable parameter는 현재 reference가 없다는 이유만으로 자동 `deferred` 처리하지 않는다. Agent Builder가 발급하는 task는 `pending|active`에서 사용자 확인을 기다리며, 사용자가 명시적으로 `defer`를 제출하고 Catalog policy가 허용한 경우에만 GraphMutation, CDS 저장과 acknowledgement 뒤 `deferred`가 된다. Permission-filtered picker는 durable credential resource/use 권한 resolver가 이미 존재하는 Mail/Gmail에만 제공한다. Gmail Draft에는 Gmail OAuth2 use-permitted credential만 노출하고 제출도 같은 조건으로 검증한다. Gmail/Mail selector는 defer할 수 없다. Slack/GitHub에는 신규 credential resource, 빈 picker 또는 credential defer 안내를 만들지 않으며 token/URL secret은 Agent Builder card의 masked control과 기존 Workflow Editor secret save 경계에서 구성한다.
...
- Slack/GitHub의 `credential_ref` ParameterTask와 빈 후보 picker는 direct-edit에서 만들지 않는다. Catalog에 정의된 channel, message, action, repository와 PR 번호는 순차 task로 제공한다. Token/URL secret도 같은 Agent Builder 설정 흐름의 masked password control로 직접 입력하되 기존 값을 hydrate하지 않고 task/session/audit에 복제하지 않는다. 적용은 기존 Workflow Editor secret save adapter와 workflow CAS를 사용하며 Node Detail 이동을 강제하지 않는다.
...
- Slack의 전송 방식과 각 parameter 설명은 Catalog의 한국어 문구로 고정한다. API mode에서는 Bot Token과 channel만, Incoming Webhook mode에서는 webhook URL만 표시·활성화한다. 전송 방식을 바꾸면 반대 mode에만 쓰이는 기존 secret과 channel 값을 canonical node data에서 제거하고, 이전 session의 잘못 활성화된 task는 reconciliation에서 `skipped`로 닫은 뒤 현재 mode의 다음 유효한 task를 활성화한다. `message`에는 생성 시 연결된 `{{result}}`가 이전 node의 `referenced_variables` 출력을 사용한다는 설명을 표시한다. 값이 없는 optional `blocks`와 `attachments`는 빈 `적용`을 `skip`으로 처리하고 완료 흐름을 막지 않는다. 기존 값 편집 중 빈 `적용`은 `clear`로 실제 graph 값을 제거한다.
...
- GitHub `comment_pr`의 `comment_body`는 Catalog 조건부 필수 parameter다. `get_pr`에서는 숨기고 preflight에서 요구하지 않으며 `comment_pr`에서는 ParameterTask와 test/deployment preflight가 모두 빈 본문을 차단한다.
...
- Session read는 완료된 persisted ParameterGroup도 현재 Catalog와 `node_id + parameter_key`로 병합한다. 기존 task 상태/version을 유지하고 신규 task만 추가하며, 사용자 입력이 필요한 첫 신규 task를 active로 만들고 group을 다시 active로 연다. 반복 read는 중복 task를 만들지 않고 `agent_builder_task=false` parameter는 제외한다.
...
- LLM 기본 설정 task와 session 복구는 GraphMutation의 `affected_node_ids`로 범위를 제한한다. 관계없는 기존 LLM에는 task를 추가하지 않는다. 미완료 session만 canonical graph에서 누락 task를 복구하고 완료 session은 새 task 때문에 다시 열지 않는다.
...
- 일반 text/JSON/reference decision이나 자연어 message에서 secret-like 값이 확인되거나 secret detector가 실패하면 fail-closed하고 기존 `400 invalid_decision`으로 거부한다. Dedicated `secret` control은 Agent Builder decision API를 호출하지 않고 기존 Workflow Editor secret save adapter로 분리한다. 거부된 입력의 원문은 graph 이외의 session/request payload, parameter task, audit, trace, log 또는 오류 message에 저장·반사하지 않는다. 일반 Catalog validation issue는 HTTP 거부가 아니라 `status=invalid`, `reason=catalog_validation_failed`인 task 결과로 저장·반환한다.
...
## Test 실행 및 secret 입력 정합성

- Catalog `secret` task는 Agent Builder card 안에 masked password control을 제공한다. UI는 기존 Workflow Editor secret node-data save adapter를 재사용하고 Node Detail 이동을 요구하지 않는다. Agent Builder ParameterTask `set` endpoint는 여전히 raw secret을 받지 않으며 조작된 요청을 `secret_forbidden`으로 거부한다. Planner, 일반 chat, GraphMutation/Agent Builder API 응답, task/session, audit, trace와 log에는 원문을 저장하거나 반사해서는 안 된다.
...
## Secret card와 이전 task action 정합성

- Slack Bot Token, Slack Incoming Webhook URL과 GitHub API Token의 Catalog `secret` task는 Agent Builder card의 masked password control로 입력해야 한다. Secret 설정만을 이유로 Node Detail 이동을 강제해서는 안 된다. 일반 Node Detail 기능과 LLM Routing 설정 이동은 유지한다.
- `이전 항목`으로 표시한 task는 canonical active task ID와 달라도 해당 task의 typed input과 허용 decision action을 제공해야 한다.
- 이전 optional task에 canonical graph 값이 없으면 `건너뛰기`를 제공하고 `skip`으로 처리해야 한다. 기존 일반 값이 있으면 `값 지우고 건너뛰기`를 제공하고 `clear`의 CAS save와 acknowledgement가 끝난 뒤에만 task를 `skipped`로 전환해야 한다. Existing secret은 raw 값 없이 존재 여부만 감지하며 같은 action은 Workflow Editor secret save/CAS로 값을 제거한 뒤 safe `skip`으로 task를 닫아야 한다.
- Required task와 `confirmation_required=true` task에는 `건너뛰기` 또는 `값 지우고 건너뛰기`를 제공해서는 안 된다. 이전 task로 이동하는 행위 자체는 graph, canonical task status/version, 다음 active task와 Workflow history를 변경해서는 안 된다.
