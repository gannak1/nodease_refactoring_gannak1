...
- task는 node id, parameter key, label, input type, required 여부, defer policy, safe description, example, validation rule reference, suggestion 목록, status, task version, stable order와 선택적 resolution source를 포함한다.
