...
- WorkflowNode의 `workflowId`와 `appId`가 함께 존재하는 결과 node data는 같은 organization, 각 resource 권한, `App.workflow_id == Workflow.id` 관계를 모두 통과해야 저장된다. 입력 순서와 무관하게 현재 node data와 patch를 합친 결과를 검증하며, 실패 시 부분 mutation을 남기지 않는다.
