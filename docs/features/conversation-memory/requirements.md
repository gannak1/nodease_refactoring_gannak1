...
- MEM-REQ-041A: Cleanup 지연과 무관하게 retention expiry에 도달한 idempotency row는 replay·conflict 대상에서 제외하고, 동일 scope/key의 새 reservation은 row lock 아래 만료 claim을 제거한 뒤 원자적으로 생성해야 한다. Periodic retention은 parent/child별 독립 quota를 유지하면서 포화된 batch를 finite per-run budget까지 반복하고 남은 backlog를 다음 schedule에 이어 처리해야 한다.
