...
- `AppService.get_user_apps`
  ...
  - Given 여러 App을 조회, When `budget_status`를 계산하면, Then workflow별 당월 비용은 grouped query로 계산하고 App별 개별 집계 쿼리를 반복하지 않는다.
...
- `AppService.list_app_operations`
  ...
  - Given operations page/batch에 여러 workflow가 포함된다, When rows를 생성하면, Then batch 단위 grouped query로 예산 상태를 계산한다.
