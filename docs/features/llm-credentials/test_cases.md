...
| Background/retry | 완료 | bounded safe reconciliation과 전역 스캔 부분 인덱스 | Log System periodic task, projection lease/retry/terminal failure, state/status + `(provider_started_at, id)` 부분 인덱스 | Log System task와 Shared batch/skip-locked/schema unit tests; PostgreSQL index 설치는 CI 실행 |
