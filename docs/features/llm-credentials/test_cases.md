...
- WorkflowEngine composition은 provider runtime과 usage recorder를 session을 미리 열지 않고 한 번 구성해 node에 주입한다. 주입 객체는 execution context, graph와 task payload에 포함되지 않으며 기존 Knowledge runtime dependency를 덮어쓰지 않는다. Runtime router는 capability-required flag의 누락과 명시적 `false`만 legacy로 선택하고 null, 숫자 또는 문자열 같은 malformed 값은 fail-closed한다.
