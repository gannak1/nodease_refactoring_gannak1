...
- Capability admission은 발급 상한 자체가 아니라 provider-independent prompt UTF-8 byte upper bound, 실제 generic `max_tokens`와 canonical model pricing의 최대 비용을 비교해야 한다. Output limit이 없으면 server cap을 적용하고 missing pricing, provider별 output-limit alias 또는 상한 초과는 provider client 생성 전에 fail-closed한다.
