...
- LLM Credentials domain은 [ADR-0064](../../decisions/ADR-0064-provider-execution-capability-boundary.md)와 [ADR-0066](../../decisions/ADR-0066-nested-llm-canonical-node-location.md)의 `ProviderExecutionCapability` schema, 발급, admission, revision과 revoke 검증의 authoritative owner다. Workflow/Conversation Memory provider call은 raw credential이나 client-selected owner가 아니라 server-issued capability를 사용해야 한다. Capability는 opaque identity/revision, organization/workflow/deployment ID/version, canonical `(container_path, node_id)`, node invocation/admission/provider-attempt binding, provider/model/credential safe reference, server-derived credential principal, credential permission decision revision, `main_generation|memory_summary` purpose, verified relation/provider-routing/pricing revision, token·cost cap과 expiry에 binding되어야 한다.
- Deployment credential policy는 graph 밖의 server-owned, revisioned row여야 하며 immutable deployment version의 canonical LLM node location마다 active policy를 최대 한 개만 허용해야 한다. Root path는 `[]`이고 V1 nested path는 ordered Loop segment만 허용한다. Organization manager만 LLM location/model에 credential을 연결할 수 있으며, manager actor는 server-derived credential principal 후보로만 사용한다. Policy write와 provider admission은 같은 organization, deployment/version, exact location, generation purpose의 active chat exact graph model, active credential, provider match, verified relation, credential `use`를 재검증하고, 정책 없음·중복·digest/path 불일치·직접 graph credential·fallback/auto-routing은 fail-closed한다.
- Main/summary provider adapter, Memory context lease, Budget reservation과 usage reconciliation은 같은 capability identity/revision과 자기 operation binding을 검증해야 한다. Capability는 short-lived single invocation scope이고 credential revoke, credential permission decision revision 변경, model relation/provider-routing/LLM transport profile fingerprint 변경 또는 expiry 뒤 새 lease claim, reservation, provider attempt admission이나 outbound call의 근거로 재사용할 수 없어야 한다. 이 fingerprint는 다른 adapter의 이관이나 cluster proxy-only enforcement 완료를 뜻하지 않는다. 이미 시작된 provider attempt의 usage reconciliation은 stale capability로 새 호출을 허용하는 것과 분리하며 [ADR-0068](../../decisions/ADR-0068-provider-usage-durable-ledger.md)의 durable operation snapshot만 사용해야 한다.
- Capability-required provider call은 final admission 뒤 canonical usage intent와 `provider_started` fence를 각각 commit한 후에만 outbound를 시작해야 한다. Ledger key는 organization/provider-attempt/purpose로 수렴하고 binding, 네 principal, revision, immutable pricing과 admitted cap이 달라진 replay는 provider 호출 전에 거부해야 한다.
- Provider 시작 뒤 timeout, response loss, malformed·상한 초과 usage 또는 terminal 저장 실패는 `outcome_unknown`으로 분류하고 같은 capability/attempt를 자동 재호출하거나 fallback하지 않아야 한다. Stale started reconciliation은 provider I/O 없이 같은 operation을 unknown으로 닫아야 한다.
- Canonical usage는 WorkflowRun 또는 compatibility projection 존재와 무관하게 보존해야 한다. `llm_usage_logs` projection은 operation당 최대 한 행이고, live credential/model/workflow/candidate가 이미 삭제됐으면 nullable reference를 제거하되 ledger의 safe snapshot을 변경하지 않아야 한다.
...
- Capability 발급과 최종 admission은 Workflow legacy/shared session과 분리한 전용 transaction에서 provider client 반환 전에 commit해야 하며 provider 호출 실패가 발급 사실을 rollback해서는 안 된다. Network I/O 중 control transaction session을 유지하지 않고 장기 provider-start/outcome/usage reconciliation은 ADR-0068 원장이 소유한다.
