...
- LLM credential config는 `LLM_CREDENTIAL_ENCRYPTION_KEYS`의 active version을 사용하는 versioned encryption envelope로 저장해야 한다. 신규·갱신 row는 평문 JSON을 저장해서는 안 된다.
...
- Gateway와 Workflow Worker는 process startup에서 LLM keyring 형식과 active version을 검증해야 한다. 두 process는 rotation 중 구키와 신키를 함께 받아야 하며 LLM credential을 사용하지 않는 Log System에는 keyring을 주입하지 않는다.
