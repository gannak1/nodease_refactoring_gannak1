...
- Capability admission은 발급 상한 자체가 아니라 provider-independent prompt UTF-8 byte upper bound, 실제 generic `max_tokens`와 canonical model pricing의 최대 비용을 비교해야 한다. `n`과 `best_of`는 정확한 정수 `1`만 허용한다. Output limit이 없으면 server cap을 적용하고 missing pricing, provider별 output-limit alias, 다중 생성 parameter 또는 상한 초과는 provider client 생성 전에 fail-closed한다.
- Policy write와 capability issue/admission의 row lock은 같은 Session identity map에 이미 적재된 ORM 객체도 `populate_existing`으로 갱신한 뒤 권한·상태·revision을 판정해야 한다. Capability 발급 만료 시각과 admission 최종 만료 판정은 동일한 PostgreSQL wall clock을 사용해야 하며 worker process clock을 TTL 권위로 사용하지 않는다.
