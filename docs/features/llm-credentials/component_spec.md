...
- `CapabilityProviderExecutionAdapter`: runtime control을 capability issue/admission command로 변환하고, 전용 session에서 authorization 재검증·config materialization·client 생성을 완료한 뒤 commit/close한다. Admission model과 user credential principal을 materialization 전에 재검증하고 capability plan 하나에서 lease를 한 번만 resolve한다.
