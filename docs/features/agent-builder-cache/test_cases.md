...
| ABC-T149c | L2 30-day expiry and bounded purge | read 또는 unexpired 같은 unique lookup key 재저장이 created_at/expires_at을 연장하지 않고 expired row는 candidate로 사용하지 않으며 batch limit 안에서 hard-delete; purge audit event와 raw row detail 없음 |
