...
- ABC-FR-099: L2 read와 같은 unique lookup key의 재저장은 기존 row의 `created_at` 또는 `expires_at`을 갱신하면 안 된다. row는 생성 시점부터 30일 뒤 만료되며 bounded background purge가 만료 row를 hard-delete해야 한다. purge는 cache key/value/plan을 audit에 복사하거나 row별 canonical audit event를 만들면 안 된다.
