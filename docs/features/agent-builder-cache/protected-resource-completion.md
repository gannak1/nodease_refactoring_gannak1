...
`AgentBuilderService`의 planning seam과 composition root에 연결하는 코드 수준 런타임 통합을 완료했다.
Cache-off는 기존 Planner 경로를 유지하고, cache hit/miss/error 진단은 allowlist된 안전한 값만 남긴다.
기능 플래그는 기본값 false이며, 별도 MBA-349 운용 증거가 없이는 production/staging serving을 활성화하지 않는다.
...
|---|---|---|---|
| 정책·식별자 | MBA-343~346 및 MBA-348 코드 수준 구현·회귀 검증; MBA-349 실제 소비 경계·운영 증거 | MBA-343 plan은 protected identity를 저장하지 않는다. MBA-344 normalizer는 protected identity를 읽지 않고 전체 safe message에서 유도한 normalized ordered typed canonical projection의 domain-separated SHA-256 `intent_signature`만 반환하며 version mismatch는 bypass한다. MBA-345~346 leaf와 MBA-348 coordinator는 Organization/user scope, selected target과 Knowledge identity를 HMAC 경계 밖으로 노출하지 않으며 serving 기본값은 비활성이다. | ADR-0063, ADR-0073, ABC343-FR-002~005, ABC343-T011~018, ABC-FR-001~008, ABC-T001~019 |
