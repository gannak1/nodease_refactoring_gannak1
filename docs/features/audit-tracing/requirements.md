...
- Model Routing Judge trace는 model, selected model, confidence, reason code, bounded usage/cost와 안전한 `reason_short`만 allowlist로 보존한다. `reason_short`는 14자 이하, 한글 포함, 제어문자 없음 조건을 통과할 때만 저장하며 raw prompt/input 또는 자유형 장문 설명을 대신 저장하지 않는다.
