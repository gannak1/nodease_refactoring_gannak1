...
- Generic 비동기 `record_audit()` 발행은 caller DB session이 있으면 같은 transaction에 `audit_event_outbox` row를 추가하고, session이 없는 legacy producer는 짧은 독립 DB transaction으로 Outbox를 확정한다. 고정된 audit event id를 idempotency key로 재사용한다. Rollout 4부터 요청 경로는 Redis/Celery `audit.record`를 직접 발행하지 않는다. Beat 기반 worker가 due/stale row를 lease하고 `AuditLog` insert와 Outbox 성공 상태를 원자 commit한다. 저장 실패는 safe reason code로 최대 5회 재시도한 뒤 dead-letter 처리하고, transaction-bound canonical AuditLog recorder는 이 generic producer 이관 대상과 구분한다.
- Workflow 실행과 연결된 AuditLog는 nullable typed `workflow_run_id`/`workflow_node_run_id`를 사용한다. 두 컬럼은 실제 Run/NodeRun FK와 조회 인덱스를 가지며 참조 실행 삭제 시 감사 행을 보존하고 FK만 NULL로 만든다. Generic Outbox와 호환 consumer는 기존 metadata correlation을 typed 컬럼으로 승격하고 NodeRun이 지정된 WorkflowRun 소속인지와 Run의 Workflow가 `audit_metadata.organization_id`와 같은 조직인지 검증한다. 조직 metadata가 없거나 malformed이거나 조직이 다르면 correlation만 제거하며, optional correlation 오류나 참조 삭제가 canonical audit 저장을 막아서는 안 된다.
