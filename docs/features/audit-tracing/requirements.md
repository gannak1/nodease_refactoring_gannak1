...
- Trace 목록의 owner metadata visibility는 app > organization > global 정책 우선순위를 먼저 해석하고 허용 App과 연결된 WorkflowRun만 SQL에서 조회해야 한다. 고정 scan limit까지 가져온 뒤 Python에서 row별 권한 필터링하지 않는다.
