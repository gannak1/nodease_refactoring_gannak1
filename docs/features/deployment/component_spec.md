...
- Logger와 Beat는 DB/Redis, Frontend server는 같은 release의 bundled internal Gateway만 사용한다. Proxy-only Frontend에서 bundled Gateway를 끄거나 `API_URL`을 다른 backend로 바꾸면 Helm validation이 실패한다. Sandbox는 일반 proxy source가 아니다. Mail IMAP은 Worker-only tunnel 안에서도 ADR-0031의 IP pinning과 hostname TLS를 유지한다. Connector DB/SSH transport도 HTTP operation policy와 분리되지만 물리적인 public socket은 `3130` tunnel로만 열고 direct fallback하지 않는다.
