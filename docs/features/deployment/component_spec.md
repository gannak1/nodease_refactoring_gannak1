...
- Gateway와 Knowledge Worker는 `3128` HTTPS CONNECT-only listener를, Workflow Worker는 `3129` HTTP-compatible listener를 사용한다. 두 포트는 `proxy-v1`의 immutable contract이며 Helm은 다른 값을 render 전에 거부한다. Workflow listener의 CONNECT는 HTTPS 443과 address-pinned IMAP 143/993만 허용한다. `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `NO_PROXY`를 Pod에 주입하지 않는다.
