...
- Gateway와 Worker는 공통 schema readiness service를 사용하고 `claim`/`drain` startup에서 Alembic head와 필수 claim column을 모두 확인한다. Alembic online migration은 같은 connection에서 bounded advisory lock을 획득하며 애플리케이션 process가 schema mutation을 수행하지 않는다.
...
- Coordinated workflow는 `claim -> disabled` 직접 전환을 거부한다. `drain -> disabled`에서는 새 Gateway image에 포함된 rollback preflight CLI를 임시 pod로 실행해 nonterminal claim, 미검토 outcome unknown, active/reserved/scheduled 전용 task가 모두 0임을 fail-closed로 확인한 뒤에만 rollout한다.
- `disabled`/`drain -> claim` 활성화는 migration 뒤 activation preflight pod를 실행한다. Legacy `workflow.execute_by_deployment`, 신규 dedicated schedule task의 active/reserved/scheduled 상태와 Redis workflow priority queue 전체 depth가 0이어야 하며 worker inspection 또는 broker queue 확인 불가 시 fail-closed한다. Queue payload 원문은 읽거나 출력하지 않는다.
