...
- Support-surface guard는 승인 workflow path allowlist와 provider-specific content signal을 함께 사용한다. 같은 content reader가 local composite action metadata도 검사하므로 승인 workflow가 `uses: ./.github/actions/...`로 provider-specific 실행을 우회할 수 없다. 모든 `.yml`/`.yaml` workflow와 composite action metadata 변경은 guard를 선택하고, guard·allowlist 변경 자체는 CI control path로서 current-head 독립 승인을 요구한다.
