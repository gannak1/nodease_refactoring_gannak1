...
- Helm 기본/production render는 disabled mode에서 통과하고 non-disabled mode에서 현재 지원하지 않는다는 safe error로 실패한다. Docker Compose에 `SCHEDULE_DISPATCH_MODE=claim|drain` 환경값이 있어도 render된 Gateway/Worker mode와 fingerprint는 disabled를 유지한다.
- CI support-surface guard는 모든 `.yml`/`.yaml` workflow 변경에서 실행한다. Legacy path, allowlist 밖으로 이름을 바꾼 workflow, 삭제 뒤 남은 stale allowlist entry, allowlist 안에 AWS credential/ECR/EKS/eksctl 신호를 넣은 workflow와 `infra/k8s`, `infra/terraform` prefix가 다시 추적되면 실패하고 현재 승인된 provider-neutral workflow 집합만 통과한다.
...
- Helm storage 기본값 `LOCAL`은 cloud 좌표 없이 통과한다. Production CLOUD는 CI 전용 non-secret placeholder로 통과하지만 unknown type, 빈 bucket, 빈 region과 과거 `gateway.env`/`worker.env` storage key를 각각 주입하면 render 전에 실패한다. Component별 storage key와 implicit region default는 없어야 한다.
