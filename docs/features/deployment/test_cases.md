...
- Production/unknown Gateway CORS는 공개 HTTPS origin만 허용하고 development/test는 loopback HTTP만 예외로 허용한다. Raw Kubernetes Gateway manifest는 이 정책과 일치하는 `NODE_ENV`를 명시하고, 잘못된 전체 origin 문자열은 startup error나 log에 포함하지 않는다.
