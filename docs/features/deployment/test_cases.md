...
- Helm 기본/production render는 disabled mode에서 통과하고 non-disabled mode에서 현재 지원하지 않는다는 safe error로 실패한다. Docker Compose에 `SCHEDULE_DISPATCH_MODE=claim|drain` 환경값이 있어도 render된 Gateway/Worker mode와 fingerprint는 disabled를 유지한다. CI support-surface guard는 `.yml`/`.yaml` 확장자와 무관하게 legacy dev namespace workflow, `deploy-eks-` workflow prefix와 `infra/k8s`, `infra/terraform` prefix가 다시 추적되면 실패한다.
