...
- CI support-surface guard는 모든 `.yml`/`.yaml` workflow와 composite action metadata 변경에서 실행한다. Legacy path, allowlist 밖으로 이름을 바꾼 workflow, 삭제 뒤 남은 stale allowlist entry, 승인 workflow 또는 `.github/actions/**/action.yml|yaml` 안에 AWS credential/ECR/EKS/eksctl 신호를 넣은 경우와 `infra/k8s`, `infra/terraform` prefix가 다시 추적되면 실패한다. Provider-neutral composite action과 현재 승인된 workflow 집합만 통과하고, non-UTF-8·symlink·과도한 크기의 executable metadata는 fail-closed한다.
