...
| Manual original content | `content_read` | active organization; document must belong to authorized manual KB | validated content response; supported PDF is inline and every original file response uses `X-Content-Type-Options: nosniff` | access path must not place content in audit/trace |
