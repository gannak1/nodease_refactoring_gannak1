...
| `reason_category` | 추천 근거의 safe category. `content_match`, `metadata_match`, `operational_fallback` 같은 allowlisted 값만 허용 |
...
`score`는 DB 저장값이 아니라 추천 요청 시점에 계산한 KB 단위 ranking 값이다. MBA-342 `parent_first_v1`은 Agent Builder backend의 같은-process 내부 service call에서만 적용한다. 이 내부 경계는 CandidateResolver-authorized 후보에 대해 semantic available이면 parent cosine relevance, unavailable이면 safe metadata relevance를 선택한다. 기존 public HTTP `POST /api/v1/knowledge/rag-recommendations`는 동일한 safe envelope를 유지하지만 semantic adapter를 구성하거나 parent 검색을 실행하지 않고 metadata-only ranking을 유지한다. Parent relevance는 cosine 최고점 0.70과 상위 최대 3개 평균 0.30으로 집계하며 추가 양수 cutoff는 없다. 선택 relevance가 0이면 최종 score는 0이고, 양수일 때만 relevance 0.70에 `source_tier`, `runtime_availability`, `sync_freshness`를 각각 0.10 비중으로 더한다. Metadata fallback 입력은 `safe_label`, `kb_safe_description`, `kb_safe_topics`에 한정한다. Collection metadata는 route/permission boundary와 `source_collection_summary`에만 사용한다. 계층형 사용자 선택 응답은 0점 후보를 표시할 수 있지만 Intent LLM에 전달하는 `knowledge_candidates`에는 `score > 0`인 후보만 최대 20개 포함한다. Parent content, document/chunk identity, raw distance와 provider payload는 내부 또는 HTTP API에 포함하지 않는다.
