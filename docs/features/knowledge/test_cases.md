...
- Demo reset은 demo 조직의 `knowledge_ingestion_outbox`를 Knowledge Base보다 먼저 삭제하고, 곧바로 upsert할 demo 조직 row 자체는 삭제하지 않는다. 미처리 색인 작업이나 새 organization-scoped 테이블이 있어도 외래키 오류 없이 reset해야 하며 오래된 outbox 작업은 남지 않아야 한다.
