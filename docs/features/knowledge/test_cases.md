...
- 비용 승인 취소, process/approval 요청 실패, `failed` 완료는 이동 intent를 만들지 않는다. 이전 KB/document의 늦은 initial fetch, SSE 또는 polling 결과는 현재 화면의 status/progress/edit gate를 변경하거나 이동을 예약하지 않는다.
