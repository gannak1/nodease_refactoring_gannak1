...
- Recommendation ranking의 `score`는 DB 저장값이 아니라 요청 시점 계산값이어야 한다. CandidateResolver-authorized parent 검색이 성공하면 cosine parent relevance만 사용하고 metadata가 더 높아도 덮어쓰지 않아야 한다. Flat/no-parent/provider·DB timeout에는 `safe_label`, `kb_safe_description`, `kb_safe_topics` metadata fallback을 사용하고 Collection metadata는 relevance에 사용하지 않아야 한다.
- Parent score는 cosine 음수와 0을 0으로 제한하고 추가 cutoff 없이 최고점 0.70과 상위 최대 3개 평균 0.30으로 결정적으로 집계해야 한다. 선택 relevance가 0이면 source tier, availability, freshness가 양수여도 최종 score가 0이어야 한다.
- Permission, active organization, lifecycle 또는 source ACL 실패는 embedding/cohort discovery/parent SQL 전에 fail-closed하고 metadata fallback으로 낮추지 않아야 한다. 권한 없는 KB ID와 exact hidden count는 query, score, response, audit, trace와 log에 나타나지 않아야 한다.
- Score-only retrieval projection과 외부 envelope에는 parent content, document/chunk identity, vector, raw distance, credential reference 또는 provider payload가 없어야 한다.
...
- Recommendation response item은 bounded `score`, `confidence`, allowlisted `reason_category`, `threshold_result`와 complete/degraded state를 포함할 수 있으며 raw retrieval/provider score나 hidden resource identity를 노출하지 않아야 한다.
