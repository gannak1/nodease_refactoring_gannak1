...
- Document settings의 PDF 원본 preview는 same-origin content API를 browser-native `object[type="application/pdf"]`로 열고, Edge에서 embed가 실패해도 같은 인증 URL을 `noopener noreferrer` 새 탭으로 열 수 있어야 한다. Local/external PDF content response는 `X-Content-Type-Options: nosniff`를 반환한다. PDF viewer를 위해 iframe sandbox에 `allow-scripts`를 추가하면 테스트 실패다.
- PDF가 아닌 FILE 원본 preview는 `allow-same-origin allow-downloads`만 허용한 기존 sandbox iframe을 유지한다. Missing KB/document id에서는 content 요청 요소를 만들지 않으며, 문서 route가 바뀌면 현재 document filename에 따라 PDF와 non-PDF viewer를 다시 선택한다.
