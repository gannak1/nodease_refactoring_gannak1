...
- FR-058: Workflow Builder용 Knowledge RAG Recommendation Adapter는 자연어 workflow intent와 LLM node purpose를 받아 safe KB recommendation과 LLM node RAG option 후보를 반환한다. Adapter는 권한을 직접 판단하지 않고 `KnowledgeCandidateResolver`의 safe candidate set만 ranking한다.
- FR-059: Recommendation item은 초기 구현에서 `candidate_type=knowledge_base`만 반환한다. Collection은 실행 candidate가 아니라 `source_collection_summary` safe metadata로만 제공하며, 현재 LLM node schema에는 `materialized_knowledge_bases`를 `knowledgeBases`로 저장한다.
- FR-060: Recommendation request의 `workflow_intent`와 `node_purpose`는 raw prompt 계열 input으로 취급한다. 길이 cap, control character normalization, durable raw storage 금지, trace/audit/log allowlist를 적용한다.
