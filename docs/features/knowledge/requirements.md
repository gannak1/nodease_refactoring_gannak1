...
- FR-078: Knowledge Base archive/delete lifecycle mutation은 endpoint에서 직접 permission cleanup, storage cleanup, retrieval exclusion, audit orchestration을 조합하지 않고 lifecycle service boundary를 통과해야 한다. Hard delete는 `team_knowledge_permissions`와 `user_knowledge_permissions` direct grant row를 같은 transaction에서 먼저 정리해 orphan permission이나 FK failure를 남기지 않아야 한다.
