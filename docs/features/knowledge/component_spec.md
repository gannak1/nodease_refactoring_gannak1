...
| Knowledge Recommendation Retrieval Port | CandidateResolver-authorized KB snapshot과 bounded safe query를 받아 model/dimension cohort별 parent cosine score를 KB 단위로 projection한다 | Workflow runtime RetrievalService, answer/citation trace와 분리한다. Parent content, document/chunk identity, vector, raw distance, credential/provider payload를 application 밖으로 반환하거나 저장하지 않는다 |
| Recommendation Embedding Credential Resolver | Active organization, exact active embedding model, verified relation과 actor `use` 권한으로 cohort credential을 결정적으로 선택한다 | 임의 provider/key fallback과 사용자 credential picker를 제공하지 않는다. unavailable/ambiguous credential은 authorized cohort metadata fallback이다 |
