...
Status: Draft
Verified Against: feature/mba-281 @ 101bde8c4b30ad84b23a79c95c6ad90f0d21443a
...
| Connection Use Resolver | Opaque Connection UUID와 execution subject user UUID를 한 query의 ID/owner predicate로 평가하고 실제 DB use에서는 row를 잠근다 | Knowledge upload/process/preview와 Gateway/Workflow Engine processor가 공유한다. KB/Collection/organization 권한을 대체하거나 organization-scoped Connection 정책을 추측하지 않는다 |
...
- DB source 설정 저장 전에는 Connection owner를 확인하고 opaque `connection_id` 외 Connection detail을 metadata에 복제하지 않는다. Background processor는 외부 connector 생성·credential 복호화·dial 전에 같은 resolver로 execution subject owner를 다시 확인하며 missing/malformed/deleted/owner-changed/non-owner를 동일한 resource-hiding 실패로 닫는다. 복호화 실패 시 저장값 fallback을 금지한다.
