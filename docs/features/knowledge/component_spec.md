...
| Document Ingestion PostgreSQL Adapter | Organization-scoped row lock, active-job unique, DB-clock execution/dispatch lease, heartbeat/fencing, canonical `KnowledgeBase -> Document -> job` claim/success/failure/recovery lock과 recovery scope `SKIP LOCKED`, retry/dead-letter/redrive와 retention query를 구현한다 | Celery task ID나 Redis 상태를 실행 권위로 사용하거나 잠긴 첫 recovery 후보를 기다리지 않는다 |
