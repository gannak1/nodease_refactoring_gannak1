...
| Knowledge Ingestion Worker | Gateway image의 parser/storage dependency로 `knowledge` queue만 소비하고 fresh authorization 뒤 job runner를 실행한다 | Workflow node queue를 소비하거나 source config를 task argument로 받지 않는다 |
