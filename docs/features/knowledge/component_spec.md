...
- DB source 설정 저장 전에는 Connection owner를 확인하고 opaque `connection_id` 외 Connection detail을 metadata에 복제하지 않는다. Background processor는 외부 connector 생성·credential 복호화·dial 전에 같은 resolver로 execution subject owner를 다시 확인하며 missing/malformed/deleted/owner-changed/non-owner를 동일한 resource-hiding 실패로 닫는다. Resolver 저장소 장애는 safe temporary failure로 정규화하고 복호화 실패 시 저장값 fallback을 금지한다. 이 resolver 조회는 runtime row lock을 소유하지 않는다.
