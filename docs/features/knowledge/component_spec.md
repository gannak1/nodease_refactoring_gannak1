...
| Document Ingestion PostgreSQL Adapter | Organization-scoped row lock, active-job unique, DB-clock claim/lease/heartbeat/fencing, retry/dead-letter/redrive와 retention query를 구현한다 | Celery task ID나 Redis 상태를 실행 권위로 사용하지 않는다 |
...
| Document Ingestion Progress Projection | Redis의 document progress key를 non-authoritative SSE 가속 projection으로 관리하고 retry/cancel/dead-letter commit 뒤 이전 attempt key를 삭제한다 | DB transaction을 소유하거나 lease를 잃은 worker가 새 attempt cache를 삭제하지 않는다 |
