...
| Document Ingestion Application | Process/sync/resume/reindex admission, same-document single-flight, duplicate resume 재사용 우선 판정, protected input revision과 atomic Document/job transaction을 조율한다 | FastAPI/Celery/SQLAlchemy concrete API를 domain policy에 넣지 않고 raw source config를 job payload로 만들거나 active 동일 intent를 변경된 Document status만으로 거부하지 않는다 |
