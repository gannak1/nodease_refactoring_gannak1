...
- organization owner/manager는 row 없이 허용되므로 이 목록에 나타나지 않는다. 빈 목록은 "부여된 App 생성 권한이 없습니다" empty state로 표시한다 (owner/manager의 기본 허용과 구분되는 안내 문구 포함).
...
- 권한 회수 처리 중: 해당 행 `회수` 버튼 비활성화(중복 클릭 방지). 404 응답(이미 회수된 권한)은 "이미 회수된 권한입니다" toast 후 목록 갱신.
