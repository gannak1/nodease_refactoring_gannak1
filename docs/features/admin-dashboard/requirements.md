...
- FR-015: 조직의 이번 달 LLM 비용 합계와 예산 위험 workflow 비율을 요약해 표시한다. 예산 사용률(당월 비용 / 예산)이 80% 이상이면 위험, 100%를 초과하면 초과로 판정한다. 비용·예산 위험은 FR-013 Security Alert와 분리한다.
