...
- 그리드 카드의 명칭은 `자동 파라미터 최적화`로 표시한다. 이 기능은 모델 라우팅이나 prompt를 변경하지 않고 응답 길이와 RAG context 설정만 점검한다.
- 카드의 상태 영역은 남은 카드 높이를 채우지 않는다. 상태, 다음 행동, 수집 진행률만 compact panel로 표시한다.
- 수집 상태: `운영 로그 수집 중 14 / 50회`처럼 대상 LLM node 전체 기준의 현재 수집 수와 점검 주기를 progress bar와 함께 보여준다. 여러 대상 노드가 있을 때 수집 수는 모든 대상 노드에 공통으로 모인 최소 실행 수다.
- 준비 상태: `다음 점검 준비 완료`를 보여 주되, 실제 추천 설정이나 예상 절감액은 목록 API가 제공하지 않으므로 표시하지 않는다.
...
- 배포가 없으면 `배포 후 자동 최적화 설정`과 비활성 설정 버튼을, 사용하지 않으면 `자동 최적화 꺼짐`과 설정 행동을 표시한다. 두 상태 모두 수집 상태와 같은 compact panel 구조를 사용해 그리드 카드의 높이와 정보 위계를 일관되게 유지한다.
- 리스트 표에서는 그리드 카드용 상세 panel을 재사용하지 않는다. 상태 badge, `수집 n / N회`, 월 검증 예산, 아이콘형 설정 행동만 한 행으로 표시하고, 표는 최소 너비를 유지해 좁은 화면에서는 가로 스크롤한다.
...
4. 50건 이상이고 로컬 라우터가 충분히 확신하면 로컬 라우터가 선택하며, 확신이 낮으면 Judge를 다시 호출한다.
