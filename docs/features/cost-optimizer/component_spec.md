...
- 입력군 목록: 한국어 이름, 변수명, source(`직접 등록`/`자동 발견`), 합성 `대표 문의`, 관찰 수, lifecycle, 고정 여부, 검증된 기본 모델 또는 `검증 대기`를 표시한다. 자동 발견 입력군도 순번이 아니라 `재무 결산·지급 승인`/`finance_closing_approval`처럼 업무 의미가 드러나는 이름과 변수명을 표시한다. 이름 생성이 아직 완료되지 않았을 때만 임시 순번 이름을 허용한다. 대표 문의가 없는 자동 발견 입력군은 `대표 문의를 준비 중입니다.`라고 표시한다. 검증된 기본 모델은 읽기 전용이며 사용자가 직접 고르지 않는다.
