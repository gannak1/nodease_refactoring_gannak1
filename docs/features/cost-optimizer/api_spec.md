...
FR-011은 `bootstrap_request_complexity_v3` 정책으로 다룬다. bootstrap API가 운영 로그
유무에 맞는 안전한 표본과 Planner 작업 난이도 프로필을 준비하고, runtime은 저장된
`economy`/`balanced`/`advanced` 등급과 0~100 점수를 평가한다. 이 프로필은 prompt, 출력
계약, RAG, downstream 계약의 복잡도를 나타내며 요청 문장 embedding이나 주제 유사도를
분류하지 않는다. 일반 실행 중 Planner/Judge LLM은 호출하지 않는다. 배포 후 replay 기반
재평가는 모델별 품질·비용 증거를 추가하는 별도 단계다.
...
| FR-011 | policy 조회/설정/refresh/preview API와 배포 후 run event가 policy table을 관리한다. 변수 치환된 prompt와 실행 입력에서 계산한 요청 난이도 점수, 전역 모델 지식, 노드별 운영 성적을 사용해 runtime 모델 선택을 제공한다. |
...
| POST | `/api/v1/workflows/{workflow_id}/llm-nodes/{node_id}/model-routing/bootstrap` | 작업 설명/기본 모델/예산으로 초안 bootstrap 작업 난이도 프로필 생성 | FR-011 | builder 이상 |
...
| FR-011 | bootstrap task complexity profile, Planner, runtime candidate scoring, bootstrap API | `apps/workflow_engine/services/model_routing_bootstrap.py`, `apps/workflow_engine/services/model_router.py`, `apps/shared/db/models/model_routing_policy.py`, `apps/gateway/api/v1/endpoints/workflow.py` | 진행중 | `apps/workflow_engine/tests/services/test_model_routing_bootstrap.py`, `test_model_routing_bootstrap_runtime.py` | unit 통과 |
...
현재 자동 모델 라우팅은 bootstrap_request_complexity_v3 전략을 사용한다. 입력군,
대표 예문, embedding vector, semantic cohort endpoint는 제공하지 않는다.

### Bootstrap Contract

초안 단계에서도 bootstrap을 만들 수 있다. bootstrap은 작업 지문을 안전하게 요약하고,
요청 난이도 분류기 artifact와 초기 후보 비교 정보를 저장한다.
...
generation_summary, stale_reason을 반환한다. 생성 요청 response에는 안전한 sample 목록도
포함될 수 있다. sample은 source, difficulty, safe_input_summary, input_length,
knowledge_enabled, output_format, planner_reason만 반환한다.
...
반환한다. decision_source는 matched_rule, default_model, fallback_model 중 하나다.

실행 trace의 llm.model_routing에는 정책 ID/version, 선택·대체 모델, strategy ID,
matched rule ID, reason code, runtime context와 안전한 decision factor만 남긴다.
judge_called는 일반 runtime에서 항상 false다. 입력군 ID, 문장 유사도, embedding vector,
대표 문장은 반환하거나 저장하지 않는다.
