...
| FR-011 | 사용자 클릭 기반 모델 라우팅 최적화 | P2 | `진행중` | `추천 분석 서비스 후속, 자동 적용 제외` | LLM 노드 상세 화면은 자동 라우팅 토글이 아니라 `모델 라우팅 최적화` 버튼을 제공한다. 사용자는 배포 후 운영 로그를 분석한 추천 모델, 예상 절감, 품질 근거를 확인한 뒤 직접 적용한다. |
...
- LLM 노드 상세 화면에는 이 노드에 대해 `모델 라우팅 최적화` 액션이 있어야 한다.
...
LLM 노드 상세 화면의 모델 라우팅 최적화 UX는 다음을 따른다.

- `모델 라우팅 최적화` 버튼은 builder 이상 사용자에게만 활성화한다.
- 버튼을 누르면 최근 배포 후 운영 로그를 분석하는 Cost Optimizer 흐름으로 이동한다.
- 기본 모델과 fallback 모델 선택 UI는 항상 유지한다. 실행 시점에 저장 모델을 자동으로 바꾸지 않는다.
- 추천 모델, 예상 비용 절감, 품질 근거, judge 필요 여부는 사용자가 확인한 뒤 직접 적용한다.
- 작업 유형 입력은 표시하지 않는다.
...
### FR-011. 사용자 클릭 기반 모델 라우팅 최적화

Cost Optimizer는 LLM 노드의 모델 라우팅과 최적화 에이전트를 단계적으로 다룰 수 있어야 한다.

모델 라우팅은 작업 난이도나 작업 유형에 따라 더 저렴한 모델 또는 더 강한 모델을 선택하는 기능이다.

현재 제품 방향은 실행 시점 자동 라우팅이 아니다. 사용자가 LLM 노드 상세 화면에서 `모델 라우팅 최적화` 버튼을 눌러 최근 배포 후 운영 로그와 후보 실험 이력을 분석하고, 추천 모델과 예상 절감, 품질 근거를 확인한 뒤 직접 적용한다.

추천 화면은 다음 전략을 제공한다.

- `자동 균형`: 비용, latency, token 변화, schema/downstream 안정성을 함께 본다.
- `비용 우선`: 품질 gate를 통과한 후보 중 비용 절감 폭을 가장 크게 본다.
- `속도 우선`: 품질 gate를 통과한 후보 중 latency 개선을 가장 크게 본다.

`throughput` 전략은 1차 범위에서 제외한다. 현재 데이터만으로 provider throughput을 안정적으로 판단하기 어렵고, 사용자가 확인 가능한 근거로 설명하기도 어렵다.

추천 상태는 다음 3단계로 나눈다.

| 상태 | 조건 | 사용자에게 보여줄 의미 | CTA |
| --- | --- | --- | --- |
| `분석 불가` | 배포 후 target LLM node의 성공 운영 baseline이 없다 | 현재 모델의 기준 비용/latency도 잡을 수 없다 | 먼저 workflow를 배포/실행해 운영 로그를 만든다 |
| `후보 검증 필요` | 운영 baseline은 있지만 성공한 대체 모델 후보 실험이 없다 | 기존 모델 로그만으로는 대체 모델 품질을 알 수 없다 | `후보 실험 실행`으로 A/B workspace에 진입한다 |
| `적용 검토 가능` | 성공한 후보 실험 중 비용이 더 낮고 schema/downstream 치명 문제가 없는 후보가 있다 | 추천 후보를 A/B 결과에서 확인하고 적용을 검토할 수 있다 | `A/B 결과에서 적용 검토`로 이동한다 |

현재 모델 로그만으로 대체 모델을 바로 추천하지 않는다. 기존 로그가 대부분 하나의 현재 모델로만 쌓이기 때문에, 다른 모델이 같은 입력에서 같은 품질을 낼지는 후보 실험 없이는 알 수 없다. 따라서 추천 후보는 후보 실험 이력 또는 별도 검증 근거가 있어야 한다.

추천 후보는 다음 gate를 통과해야 한다.

- 후보 실행 상태가 `success`다.
- `schema_status`가 `failed`가 아니다.
- `downstream_state`가 `incompatible`이 아니다.
- 후보 비용이 baseline 비용보다 낮다.
- 선택한 전략의 score에서 가장 높은 후보다.

추천 분석은 workflow 전체 실행 횟수가 아니라 target LLM node 기준 실행 이력을 사용한다. 분석에는 배포 후 운영 실행만 사용한다. `deployment_id IS NULL`인 테스트 실행, `trigger_mode=test/manual` 성격의 배포 전 실행, `cost_optimizer_compare` 실행은 추천 근거에서 제외하고 후보 탐색/검토 자료로만 사용한다. 보관 기간 만료나 redaction 때문에 usage/output을 복원할 수 없는 run은 추천 프로파일 계산에서 제외한다.

추천 분석 결과는 최소한 다음 정보를 제공해야 한다.
...
- fallback/retry 추세
- judge 평가 필요 여부와 judge sampling 정책
- 추천을 적용하지 않아야 하는 경고 사유

추천 모델을 현재 노드에 적용하는 작업은 자동으로 수행하지 않는다. 사용자는 추천 근거와 A/B 비교 결과를 확인한 뒤 `현재 노드에 적용` 액션을 명시적으로 눌러야 한다. 현재 프론트 구현은 모델 라우팅 추천 화면에서 직접 apply하지 않고, 기존 A/B 결과 화면으로 이동해 후보 결과를 다시 확인하게 한다.

Judge는 모든 운영 실행마다 호출하지 않는다. Judge score는 모델 전환 직후, confidence가 낮은 후보, schema가 없는 출력, downstream 실패 의심, 고위험 workflow, 표본 추출 대상 같은 경우에만 비동기로 수집한다. Judge 비용은 절감 효과 계산에 포함해야 하며, judge 비용을 반영한 순절감이 낮으면 추천하지 않는다.

추천 결과는 설명 가능해야 한다. 추천 summary에는 최소한 다음 정보를 safe summary로 남긴다.
...
```json
{
  ...
  "estimated_cost_reduction_rate": 0.76,
  "quality_basis": {
    "schema_pass_rate": 0.98,
    "downstream_success_rate": 1.0,
    "sample_count": 90
  },
  "judge_policy": "sampled_async",
  "reason": "최근 배포 후 운영 로그에서 품질 gate를 통과했고 평균 비용이 76% 낮습니다."
}
```

작업 유형은 사용자가 직접 고르는 입력값으로 두지 않는다. 추천 분석은 node 설정, output format/schema, prompt, RAG 사용 여부, downstream 계약, 과거 node run profile을 보고 내부적으로 판단한다.

품질 gate를 통과하지 못하면 비용이 더 싼 모델을 추천하지 않는다. 이 기능의 기본 원칙은 `품질 유지 후 비용 절감`이다.
