...
Status: Draft
Related Features: llm-credentials, workflow, deployment, admin-dashboard

## Purpose

workflow의 현재 모델과 더 저렴한 후보 모델을 실제 실행으로 비교해, 비용 절감과 품질 트레이드오프를 근거로 제시한다. [PRD](../../PRD.md)의 FR-021~FR-022를 담당한다.

모델 비교 실행 API(`POST /api/v1/workflows/{workflow_id}/compare`)는 구현돼 있으며, 이 feature의 범위는 비교를 "비용 최적화" 흐름으로 묶는 UI와 추천 리포트다.

## User Stories

- 플랫폼 관리자로서, 비용이 큰 workflow에서 "비용 최적화"를 실행해 더 싼 모델과의 비교 결과를 보고 싶다.
