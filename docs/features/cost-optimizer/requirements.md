...
- 비교 리포트에는 context token estimate, retrieved chunk count, citation count, cost, latency, policy result 같은 safe summary만 표시한다. 권한 없는 문서명/ID, raw source metadata, raw prompt/completion, raw chunk content는 표시하지 않는다.
