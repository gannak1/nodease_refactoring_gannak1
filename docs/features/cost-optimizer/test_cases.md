...
| FR-011 | Frontend component | `apps/client/app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | `모델 라우팅 최적화` 진입 버튼, 자동 라우팅 토글, ON/OFF 전환, 규칙 미일치 시 기본 모델/대체 모델 저장, 입력군 관리 우선 배치, 점검 주기/월간 검증 예산/입력군 최대 개수 slider, 대표 문의 표시, 배포 전 draft 입력군 생성·수정·삭제, manual 수정, auto→manual 전환, 대표 문의 마법사와 직접 등록 API 호출 | 작성 완료 | `cd apps/client && npm run test -- --run app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | 이번 변경 targeted test |
...
| FR-011 | Adaptive cohort service | `apps/workflow_engine/tests/services/test_model_routing_adaptive_cohorts.py` | 자동 발견, 최대 개수, dormant/revive/retire lifecycle, 직접 등록한 cohort의 capacity 규칙, manual 대표 문의 수정 시 재임베딩/evidence 만료와 수정 전 observation 매칭 해제 | 작성 완료 | targeted pytest | 이번 변경 targeted test |
| FR-011 | Adaptive validation | `apps/workflow_engine/tests/services/test_model_routing_adaptive_validation.py`, `test_model_routing_validation_planner.py`, `test_model_routing_adaptive_policy.py` | 입력군별 5회 Replay/Judge gate, actual-model/fallback guard, 검증 통과 rule 투영 | 작성 완료 | targeted pytest | 통과 |
| FR-011 | Gateway cohort API | `apps/gateway/tests/api/cost_optimizer/test_cost_optimizer_api.py` | 대표 문의 wizard, policy row가 없는 workflow draft의 입력군 생성·조회·수정·삭제, 직접 입력군 embedding/저장, 대표 문의가 포함된 policy response summary, manual 수정 API와 auto→manual 동일 row 전환 API | 작성 완료 | `apps/gateway/.venv/Scripts/python.exe -m pytest apps/gateway/tests/api/cost_optimizer/test_cost_optimizer_api.py` | 이번 변경 targeted test |
...
| FR-011 | 50 deployed query experiment | `scripts/experiments/model_routing/adaptive_model_routing_50_holdout.py` | 실제 배포 40건으로 자동 입력군을 발견하고, 입력군별 후보 실제 Replay 5회와 LLM Judge gate를 마친 뒤, 겹치지 않는 holdout 50건의 cohort/model/trace/fallback을 보고서로 저장 | 작성 완료 | `NODEASE_MODEL_ROUTING_WEBHOOK_SECRET` 환경변수와 `--confirm-live --reset-adaptive-state` 실제 Provider 실행. Secret은 DB 평문 컬럼에서 역조회하지 않음 | 50/50 성공, trace 50/50, 자동 입력군 2개는 `gpt-5.4-nano` 40건, 고위험 10건은 `gpt-4.1`, fallback 0. 보고서 경로는 `reports/model-routing/adaptive-routing-run-b46d0c6d30924f1bbde73c9857fe1969.md` |
...
| FR-011-A45 | manual cohort wizard | builder가 대표 문의 하나만 입력했다 | wizard를 실행한다 | 이름, 영문 key와 중복 없는 합성 예문 3~5개를 반환한다. 사용자는 예문을 삭제할 수 있으며 policy rule과 selected model은 변경하지 않는다. |
...
| FR-011-A50 | draft cohort materialization | 배포 snapshot에 직접 입력군 초안 3개가 있고 첫 terminal 운영 실행이 완료됐다 | bootstrap policy를 만들고 입력군을 승격한다 | 초안 UUID를 보존한 `source=manual`, `status=proposed` DB row와 대표 문의·합성 예문 각각의 embedding을 만든다. embedding이 일시 실패하면 run은 성공하고 다음 성공 운영 실행에서 다시 시도한다. |
...
| FR-011-A53 | adaptive route match boundary | 자동 발견 입력군과 직접 등록 입력군에 각각 대표·최근 관찰 vector가 있고 다른 업무 문의의 최고 유사도가 0.50 초반이다 | 적응형 runtime catalog와 observation matcher를 실행한다 | 발견 군집은 0.50을 쓸 수 있지만 자동 route는 0.55, 직접 등록 route는 0.60 미만을 확정하지 않는다. 각 입력군의 상위 2개 유사도 평균을 비교하고 1·2위 최소 점수 차이를 적용한다. |
| FR-011-A55 | multiple representative examples | 직접 입력군에 대표 문의와 합성 예문 2~4개가 저장됐다 | 운영 문의를 관찰 저장 및 runtime에서 판정한다 | 입력군별 예문 유사도 상위 2개 평균으로 동일한 cohort를 선택하고, 예문 하나의 우연한 고점만으로 확정하지 않는다. |
...
| FR-011-R22 | deployed bootstrap | 자동 라우팅 ON인 배포 LLM node에 아직 policy row가 없다 | runtime이 첫 실행 모델을 해석한다 | graph의 legacy policy snapshot을 평가하지 않고 node의 저장 `model_id`/`fallback_model_id`를 사용한다. terminal workflow 완료 뒤 생성된 policy row도 같은 모델을 보존하고 rule 없이 시작한다. |
...
| FR-011-R25 | deployment bootstrap boundary | draft에서만 자동 라우팅을 ON으로 저장했고 deployment snapshot에는 해당 설정이 없다 | 운영 실행을 완료한다 | policy row/event를 만들거나 refresh를 예약하지 않는다. 자동 라우팅 설정을 포함해 배포한 뒤 target LLM node가 성공한 terminal workflow 완료부터 policy row와 표본 집계를 시작한다. |
...
| FR-011-P01 | preview matched rule | active deployment policy에 short 입력 rule이 있고 실행 주체가 rule model을 사용할 수 있다 | preview API를 호출한다 | runtime과 같은 `ModelRouter.resolve_policy()`가 rule model과 cohort/rule/reason safe summary를 반환한다. |
