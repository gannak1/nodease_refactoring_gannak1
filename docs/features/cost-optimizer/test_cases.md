...
| FR-011-A69 | 80-case four-arm economics benchmark | 중복 없는 synthetic 요청 80개가 입력 구조 4종과 길이 4단계에 각각 20개씩 배분되고, economy·balanced·advanced 난이도와 8개 이상의 업무 영역을 포함한다 | 같은 요청을 고가 `gpt-5.4`, 중가 `gpt-5.4-mini`, 저가 `gpt-4o-mini`, 자동 `judge_bootstrap_incremental_v1` arm에 각각 실행하고 모델·arm·비용·순서를 숨긴 품질 Judge로 네 출력을 평가한다 | 총 320개 workflow 실행의 품질, 비용, task/end-to-end 시간, JSON 계약, workflow 성공, 선택 모델, Judge 호출을 기록한다. 고가 고정 대비 절감액·절감률, 품질 손실, 지연 변화, 심각 회귀, 품질 1점당 절감액을 전체·난이도·구조·길이별로 보고하고 명시된 gate로 trade-off를 판정한다. |
| FR-011-A70 | runtime Judge display-reason normalization | Judge가 허용된 모델과 0~1 신뢰도를 반환했지만 `reason_short`가 14자를 넘거나 한국어가 아니다 | runtime Judge 응답을 검증한다 | 유효한 모델 선택을 버리지 않고 `reason_code`에 대응하는 안전한 14자 이하 한국어 문구로 표시 이유만 정규화한다. 원래의 계약 밖 문구는 trace와 학습 artifact에 남기지 않는다. |
| FR-011-A71 | price-tier qualification dataset | 최종 80건과 문장이 겹치지 않는 synthetic 요청 16건이 있다 | 일반 workflow chat 모델의 가격군 대표 예선을 준비한다 | 입력 구조 4종과 길이 4종이 각각 4건이고, 난이도 3종과 업무 영역 8종 이상을 포함한다. `-pro` 초고가 모델과 권한·credential·workflow chat filter를 통과하지 못한 모델은 예선에서 제외한다. |
| FR-011-A72 | price-tier representative selection | 가격군별 모델이 같은 16건 workflow와 blind 품질 Judge 평가를 완료했다 | 저가·중가·고가 대표를 선발한다 | workflow 성공률과 schema 통과율 90% 이상, 품질 계약 통과율 80% 이상인 후보 중 최고 평균 품질 3점 이내 후보만 남기고 실제 평균 비용이 가장 낮은 모델을 대표로 선택한다. 비용이 같으면 평균 latency가 낮은 모델을 선택한다. 어느 가격군도 대표가 없으면 최종 80건 실행을 차단한다. |
| FR-011-A73 | qualified full-catalog four-arm benchmark | 가격군 대표 3종과 초고가군을 제외한 예선 대상 전체 모델이 확정됐다 | 기존 80건을 대표 고가·중가·저가 고정과 전체 후보 자동 라우팅으로 실행한다 | 예선과 최종 실행 비용을 분리하고, 기존 4-arm 보고서와 같은 품질·비용·시간·계약·심각 회귀 gate로 trade-off를 판정한다. |
| FR-011-A74 | qualified candidate runtime enforcement | 예선 gate를 통과한 자동 라우팅 후보 목록이 실험 실행 조건으로 주어졌다 | 자동 routing policy를 생성하거나 갱신한다 | `active_policy.candidate_model_ids`와 profile catalog가 같은 후보만 포함한다. 계정에서 실행 가능한 다른 모델이 있어도 Runtime Judge 후보와 실제 선택 결과에 나타나지 않는다. |
