...
| --- | --- | --- | --- | --- | --- |
| FR-001 | LLM node detail action | GET availability | LLM 노드에서만 A/B 테스트 진입 가능 | 작성 전 | 미실행 |
...
| --- | --- | --- | --- | --- | --- | --- |
| FR-001 | Frontend component | `apps/client/app/features/workflow/tests/costOptimizerEntry.test.tsx` | LLM 노드 전용 진입 액션, non-LLM 차단 | 작성 전 | `cd apps/client && npm run test` | 미실행 |
| FR-001 | Gateway API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | availability, not-LLM, not-found | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-002 | Gateway API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | latest/list baseline query | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-003 | Gateway API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | candidate schema validation, unavailable model | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-004 | Gateway service/API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | baseline input restore, wrong baseline scope | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-005 | Gateway service/API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | A 미재실행, B만 실행 | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-006 | Gateway API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | compare response shape, safe trace | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-007 | Gateway service/API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | compatible/warning/incompatible 판정 | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-008 | Gateway API | `apps/gateway/tests/api/test_cost_optimizer_api.py` | apply, draft conflict, downstream ack | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
| FR-009 | Gateway/service | `apps/gateway/tests/api/test_cost_optimizer_api.py` | `llm_usage_logs` 기록, secret redaction | 작성 전 | `cd apps/gateway && PYTHONPATH=$(git rev-parse --show-toplevel) .venv/bin/python -m pytest tests/api/test_cost_optimizer_api.py` | 미실행 |
...
- `GET /baselines/latest?status=success`는 target LLM node의 가장 최근 성공 로그를 반환한다.
