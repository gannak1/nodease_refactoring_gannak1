...
| FR-011 | Frontend component | `apps/client/app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | `모델 라우팅 최적화` 진입 버튼, 자동 라우팅 토글, ON/OFF 전환, 규칙 미일치 시 기본 모델/대체 모델 저장, 입력군 관리 우선 배치, 점검 주기/월간 검증 예산/입력군 최대 개수 slider, 대표 문의 표시, manual 수정, auto→manual 전환, 대표 문의 마법사와 직접 등록 API 호출 | 작성 완료 | `cd apps/client && npm run test -- --run app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | 이번 변경 targeted test |
...
| FR-011-A48 | cohort lifecycle | 자동 입력군 traffic share가 3개 점검 구간 연속 5% 이하다 | lifecycle을 갱신한다 | `dormant`가 되고 이후 10% 이상으로 회복하면 `active`, 90일 동안 휴면이면 `retired`가 된다. 직접 등록/필수/안전 보호 입력군은 자동 휴면 처리하지 않는다. |
