...
| FR-011 | Adaptive validation | `apps/workflow_engine/tests/services/test_model_routing_adaptive_validation.py`, `test_model_routing_validation_planner.py`, `test_model_routing_adaptive_policy.py` | 배포 직후 대표 예시 1~5개 bootstrap, 운영 입력군별 5회 Replay/Judge gate, actual-model/fallback guard, 검증 통과 rule 투영 | 작성 완료 | targeted pytest | 통과 |
...
| FR-011-A67 | factual reliability judge | authoritative evidence가 없는 동일 입력에 한 출력은 확인되지 않은 제품 메뉴·정책을 단정하고 다른 출력은 근거 부족을 명시한다 | blind 품질 Judge를 실행한다 | `factual_reliability`가 필수 dimension으로 포함되고, 근거 없는 단정은 감점하되 안전한 불확실성 표현 자체는 감점하지 않는다. |
