...
| M365-F08A1 | Profile author·production approver와 분리된 holdout operator가 exact proposed profile/source manifest, immutable holdout dataset/evaluator/metric/threshold와 budget·credential·egress로 `activation_holdout_evaluate` 제출 | Run intent/accepted receipt/audit commit 뒤에만 ADR-0069 attempt를 실행하고 registered evaluator finalizer가 terminal valid/invalid/incomplete artifact, receipt와 audit를 원자 publication | Activation·Benchmark 후속 |
...
| M365-F08A3 | 같은 holdout command ID·actor·canonical request 재전달 또는 같은 command/artifact identity를 다른 profile/source/dataset/evaluator/threshold digest로 재사용 | Exact replay는 기존 run/artifact/receipt 반환. 다른 digest는 `model_routing.activation_holdout_command_conflict`와 모든 success write/provider I/O zero-write | Activation·Benchmark 후속 |
| M365-F08A4 | 같은 holdout run의 terminal finalizer가 순차·동시 재전달되거나 artifact/receipt/audit 중 하나가 실패 | Unique identity/CAS로 terminal artifact와 audit 각각 1회에 수렴. Transaction 실패는 artifact/terminal receipt/audit 모두 rollback | Activation·Benchmark·Ledger 후속 |
| M365-F08B | `proposed` profile의 manifest/locked holdout, credential policy, Worker와 rollback readiness가 통과하고 작성자와 다른 actor가 current expected domain generation·role revision으로 승인 | Guard lock 아래 다음 profile-bound safety generation을 한 번 할당하고 profile을 `limited_canary`로 전이하며 exact profile/revision/generation의 `canary_successor` role과 stable canary capability를 원자 설정. 기존 `production_route`는 유지하고 최초 rollout의 빈 route만 exact current-valid non-V2 rollback policy로 초기화. Non-target production scope의 V2 실행 0회 | Activation 후속 |
...
| M365-F08D0 | Pre-registered current/next canary window schedule에서 current aggregate revision과 모든 source-instance watermark가 cutoff를 충족하고 evidence system이 `seal_canary_evidence_window`를 제출 | Schedule/pointer/current aggregate/watermark lock 아래 current window sealed state, immutable snapshot ID/revision/hash, registered next-window로의 pointer 전환, receipt와 canonical audit를 한 번 원자 commit. Same request replay는 기존 snapshot 반환 | Activation·Benchmark 후속 |
| M365-F08D0A | Seal command의 expected current aggregate revision을 읽은 뒤 server-derived authoritative event time이 cutoff 이전인 evidence append가 먼저 current window revision을 증가시킴 | Seal은 stale revision과 snapshot/pointer/receipt/success audit zero-write. Current revision으로 재제출한 seal은 current assigned event를 포함한 snapshot을 생성 | Activation·Benchmark 후속 |
...
| M365-F08D0B1 | Authoritative event time이 cutoff 이후인 non-blocking event가 current window seal보다 먼저 도착 | Server는 immutable `[start, cutoff)` schedule로 assigned window를 next window로 결정한다. Current aggregate revision과 snapshot membership 불변이며 이후 seal은 이 event를 포함하지 않는다 | Activation·Benchmark 후속 |
...
| M365-F08D0E | 같은 watermark command의 다른 source/barrier payload, stale expected revision, barrier regression 또는 terminal append receipt 누락 | Payload conflict는 `canary_watermark_command_conflict`, stale/regression/incomplete coverage는 `canary_watermark_stale`; watermark/receipt/success audit zero-write | Activation·Benchmark 후속 |
...
| M365-F08D2 | 같은 environment/family/profile revision/safety generation/source kind/opaque source instance/source event ID가 재전달되고 authoritative event time, assigned window, metric contract, event kind와 payload digest가 같거나 다름 | Unique source identity는 source instance와 instance-local event ID를 포함하고 canary window/comparison field는 제외한다. 모든 comparison field까지 같으면 기존 event/receipt를 반환하고 aggregate revision/audit를 한 번만 기록한다. 같은 instance event의 window 재분류 또는 mismatch는 `model_routing.canary_evidence_conflict`와 zero-write | Activation·Benchmark 후속 |
| M365-F08D2A | 같은 source kind에 등록된 서로 다른 opaque source instance 두 개가 각 namespace에서 동일한 event ID와 유효한 증명을 제출 | Source instance가 identity에 포함되므로 두 event를 각각 해당 assigned window에 한 번 집계한다. Cross-instance event ID 일치만으로 dedupe/conflict하지 않고 source instance 원문은 trace/audit에 노출하지 않는다 | Activation·Benchmark 후속 |
...
| M365-F18H | 최초 rollout family라 production predecessor와 `production_route`가 없는 상태에서 limited canary와 production promote를 순서대로 수행 | Canary start는 `canary_successor` role을 설정하고 빈 `production_route`를 profile에 고정한 exact current-valid non-V2 rollback policy로 초기화해 non-target만 그 route를 사용한다. Promote는 production route를 successor로 전환하고 canary role을 비우며 successor만 `production_active`로 만들고 존재하지 않는 predecessor update/audit를 생성하지 않는다 | Activation 후속 |
...
| M365-F22C | 같은 profile lifecycle command ID를 profile/transition/scope/reason/expected profile·family·domain-role/domain generation/sealed snapshot·`expected_snapshot_invalidation_revision` 또는 actor가 다른 요청에 재사용 | Receipt actor/hash 불일치로 `model_routing.activation_command_conflict`; profile/domain/generation/receipt/success audit zero-write. Same actor/hash replay만 기존 receipt 반환 | Activation 후속 |
| M365-F22D | 같은 command ID를 다른 actor/canonical request에 재사용하는 family create, global kill, holdout, canary evidence, watermark, seal, benchmark, workflow policy와 일반 profile lifecycle 각각 실행 | Command registry가 각 command-specific conflict code를 반환하고 일반 profile lifecycle에만 `activation_command_conflict`를 사용한다. 모든 mutation/receipt/success audit/provider I/O zero-write | Activation·Benchmark 후속 |
