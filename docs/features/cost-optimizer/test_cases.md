...
| FR-012-R06a | Responses runtime compatibility | GPT-5/o LLM node에 `top_p`, `presence_penalty`, `frequency_penalty`, `stop`이 저장되어 있다 | OpenAI client가 동기 또는 비동기 Responses 요청을 만든다 | 저장된 node parameter는 유지하고, 외부 요청에서는 지원하지 않는 parameter를 제외한다. |
