...
| M365-F08D0C | 등록 source adapter가 current open-window ID, 자기 source의 monotonic barrier/position, observed-through, expected revision과 terminal append receipt coverage로 `advance_canary_source_watermark` 제출 | Window pointer/source binding/coverage를 lock 안에서 재검증하고 watermark revision, actor/request receipt와 redacted audit를 한 번 원자 commit | Activation·Benchmark 후속 |
