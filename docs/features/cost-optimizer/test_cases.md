...
- RAG strategy 비교 summary builder는 `retrieval_strategy`, `rag_mode`, selected collection/KB count, retrieved chunk count, citation count, context token estimate, cost, latency, policy result만 사용하고 raw source metadata나 raw chunk content를 포함하지 않는다.
