...
| FR-011 | 50+ deployed query experiment | `scripts/experiment_semantic_model_routing.py`, `apps/workflow_engine/tests/e2e/test_semantic_model_routing_experiment.py` | 실제 배포 endpoint에 대표 문장과 겹치지 않는 60개 입력을 보내 예상/실제 cohort·model·reason과 안전 지표를 보고서로 저장. 같은 실험 ID 재사용 시 현재 요청 이전 run을 제외 | 작성 완료 | 실제 배포 수동 검증 + script contract pytest | 입력군 95.0%(57/60), 모델 96.7%(58/60), 고위험 100%(20/20), trace 100%(60/60), script tests 12개 통과 |
