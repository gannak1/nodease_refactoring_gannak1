...
| FR-014 | frontend deployment/manage | `apps/client/app/features/workflow/components/deployment/DeploymentFlowModal.test.tsx`, `apps/client/app/features/workflow/components/deployment/AutomaticOptimizationManagementModal.test.tsx`, `apps/client/app/features/workflow/tests/costOptimizer/fr14-dashboard-automatic-optimization-management.test.tsx` | 배포 3단계에서 자동 최적화 설정 전달, 워크플로우 리스트 보기의 배포별 `관리`가 정확한 deployment 설정을 조회해 관리 modal을 연다, 그리드 카드는 자동 최적화 영역을 숨긴다, 관리 modal에서 대상 node를 보존한 채 주기/예산 수정 | 작성 완료 | targeted Vitest | 통과 |
