...
| FR-011-A45 | manual cohort wizard | builder가 대표 문의 하나만 입력했다 | wizard를 실행한다 | 이름과 영문 key 초안을 반환하되, policy rule과 selected model은 변경하지 않는다. |
...
| FR-011-A50 | draft cohort materialization | 배포 snapshot에 직접 입력군 초안 3개가 있고 첫 terminal 운영 실행이 완료됐다 | bootstrap policy를 만들고 입력군을 승격한다 | 초안 UUID를 보존한 `source=manual`, `status=proposed` DB row와 대표 문의 embedding을 만든다. embedding이 일시 실패하면 run은 성공하고 다음 성공 운영 실행에서 다시 시도한다. |
...
| FR-011-A53 | adaptive route match boundary | 자동 발견 입력군과 직접 등록 입력군에 각각 대표·최근 관찰 vector가 있고 다른 업무 문의의 최고 유사도가 0.50 초반이다 | 적응형 runtime catalog와 observation matcher를 실행한다 | 발견 군집은 0.50을 쓸 수 있지만 자동 route는 0.55, 직접 등록 route는 0.60 미만을 확정하지 않는다. 각 route는 최대 8개 대표 vector 중 최고점을 사용하고 전역 top-k 독점을 허용하지 않는다. |
