...
| FR-011 | Frontend component | `apps/client/app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | 자동 라우팅 토글, 기본/fallback 모델, 점검 주기 저장, prior-guided profile 표시, 입력군·검증 예산 UI 미노출 | 작성 완료 | `cd apps/client && npm run test -- --run app/features/workflow/tests/costOptimizer/fr3-llm-node-routing.test.tsx` | 통과 |
