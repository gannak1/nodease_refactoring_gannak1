...
| FR-011-P05 | preview feature parity | 배포 node에 작업 설명과 고정 prompt가 있고 preview 입력이 있다 | preview API를 호출한다 | runtime과 같은 `routing_feature_text()` builder가 현재 입력과 작업 계약을 resolver에 전달한다. Preview는 실제 retrieval을 수행하거나 RAG runtime signal을 합성하지 않는다. |
