...
- BGT-REQ-011: 사용률은 `당월 비용 합계 / monthly_budget_usd`로 계산한다. 당월 비용 합계는 해당 workflow의 `llm_usage_logs.total_cost`를 KST(Asia/Seoul) 달력 월 경계로 합산한 값이며, usage row가 없거나 `total_cost`가 NULL인 row는 0으로 합산한다 (admin-dashboard 시간대/집계 규칙과 동일). 실행 차단 판정의 비용 합산 범위는 `workflow_id`와 월 경계로 결정한다. 예산 row 조회와 접근 권한은 organization scope로 제한하지만, `llm_usage_logs.organization_id`는 nullable/denormalized 호환 컬럼이므로 NULL row를 제외하는 필수 조건으로 사용하지 않는다. 따라서 `workflow_id`가 일치하는 기존/마이그레이션 로그는 `organization_id`가 NULL이어도 합산한다. 관리자 projection은 BGT-REQ-020의 더 엄격한 tenant visibility 조건을 적용하며, 이 조건이 실행 차단 판정의 합산 범위를 완화하지 않는다.
