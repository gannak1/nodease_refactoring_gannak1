...
- BGT-REQ-039B (Query Embedding Target Extension): RAG query embedding은 distinct canonical embedding model마다 ADR-0065의 capability identity/revision, organization billing principal, pricing snapshot, `output_token_cap=0`과 stable provider attempt에 binding된 MBA-287 durable provider operation을 사용해야 한다. Intent와 `provider_started`가 provider 전송 전에 확정되지 않거나 usage ledger가 주입되지 않으면 provider를 호출하지 않는다. Raw query와 vector는 reservation, usage 또는 reconciliation data에 저장하지 않는다.
