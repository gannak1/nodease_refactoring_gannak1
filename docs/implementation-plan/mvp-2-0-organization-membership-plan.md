...
Source of Truth: Yes
Verified Against: origin/dev @ cde421f2cbd98d0ede4e00cac2150ece36e413bd
...
| active organization 계약 유지 | MVP 2-0만으로 header/session 기반 active organization 방식을 강제 도입하지 않는다. 기존 fallback을 membership 기반으로 바꾸되 API 계약 변경은 별도 ADR 결정 후 진행한다. |
...
- active organization 변경은 기존 ADR의 header/session 결정과 충돌하지 않게 API 계약을 먼저 확정한다.
...
MVP 2-0 완료 전에는 MVP 2의 knowledge/RAG 권한 구현을 시작하지 않는다.
...
| `docs/decisions/ADR-202606271559-active-organization.md` | team membership fallback 대신 organization membership 기준으로 ADR 갱신 |
...
| active organization ADR과 충돌 | client/server 계약 혼란 | organization membership 기준으로 ADR 갱신 후 header/session 방식 결정 |
...
MVP 2 본작업은 Issue 0-2와 Issue 0-4가 끝난 뒤 시작한다.
