...
### `DELETE /api/v1/permissions/llm-credentials/{credential_id}/users/{user_id}`

User direct LLM credential 권한을 회수하는 API다.
...
| 대상 user가 없거나 organization scope 안에 없음 | `404` | `resource.not_found` |
| credential `manage` 또는 organization `manager` 권한 없음 | `403` | `permission.denied` |
| 회수할 user LLM credential permission row가 없음 | `404` | `resource.not_found` |
