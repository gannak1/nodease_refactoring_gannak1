...
- Backend 기준 `POST /api/v1/apps`, `GET /api/v1/apps`, `POST /api/v1/apps/{app_id}/clone`, `POST /api/v1/workflows`는 `X-Organization-Id` header로 active organization을 명시한다. MBA-71 프론트 변경 기준 app wrapper는 공통 `apiClient`로 이 header를 첨부하며, workflow wrapper는 별도 확인이 필요하다.
