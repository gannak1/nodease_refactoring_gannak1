...
- 상태 전이 기준은 `requested -> running -> completed|failed|cancelled|blocked`이다. Request schema validation, invalid `correlation_id`, invalid organization header, missing required header/body field처럼 answer 실행 시작 전 확인되는 400/422 오류는 row를 만들지 않는다. KB 없음, inactive KB, scope 밖 KB, organization mismatch처럼 `404 resource.not_found`로 숨겨야 하는 경우도 resource hiding을 유지하기 위해 row를 만들지 않는다. Schema validation, organization header validation, active organization scope 확인, KB scope visibility 확인을 모두 통과한 뒤 row를 생성하고 `requested`로 시작한다. Retrieval/generation을 시작하기 전에 `running`으로 전환한다. 정상 종료는 `completed`, provider 또는 내부 오류는 `failed`, client disconnect 또는 명시 취소는 `cancelled`, scope 안 resource에 대한 policy/permission preflight 차단은 `blocked`로 닫는다.
