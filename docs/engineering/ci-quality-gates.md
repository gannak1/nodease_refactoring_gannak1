...
파일명 token만으로 찾기 어려운 Shared 응답 schema의 간접 소비자는 검토 가능한 명시적 source-to-test mapping으로 보완한다. 매핑된 test가 rename 또는 삭제되어 존재하지 않으면 조용히 제외하지 않고 selector가 실패해야 하며, mapping 변경에는 실제 source 입력에서 모든 소비자 test가 선택되는 단위 테스트를 함께 둔다.
