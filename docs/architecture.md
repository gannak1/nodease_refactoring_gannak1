...
| Knowledge Collection Sync Request Application | Gateway에서 active organization과 `sync`/`sync_manage`, Manual Collection/source eligibility, idempotency/single-flight를 검증하고 per-target revision을 포함한 durable job snapshot/audit를 원자 저장한 뒤 job UUID만 Celery에 발행한다. |
| Knowledge Collection Sync Worker Application | Workflow Engine에서 fresh worker-start 권한 재검사, PostgreSQL lease, deterministic bounded DB target batch, original-total reconciliation, retry/partial/terminal 집계와 stale recovery를 수행한다. Document adapter는 source read 전에 shared advisory lock과 target revision gate를 통과하고 canonical versioned finalizer로 active pointer를 교체한다. |
