...
- Workflow Worker egress NetworkPolicy는 DNS, configured DB port, Redis, Sandbox와 internal Squid `:3129`만 허용하며 public 80/143/443/993 direct route를 두지 않는다. Gateway와 Knowledge Worker도 Squid `:3128`만 사용한다. Squid는 private·metadata 범위를 제외한 public 80/443과 Worker-only IMAP CONNECT 143/993을 사용할 수 있다. Standard NetworkPolicy는 additive이고 CNI가 집행해야 하므로 pinned Calico CI와 실제 배포 CNI의 negative probe를 모두 통과해야 proxy-only 활성화를 완료할 수 있다.
