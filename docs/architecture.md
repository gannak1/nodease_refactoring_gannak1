...
- App hard delete는 [ADR-0048](decisions/ADR-0048-app-workflow-deletion-and-operational-retention.md)의 lifecycle application boundary가 소유한다. Endpoint나 ORM cascade 하나에 맡기지 않고 App/canonical Workflow lock, 잠금 뒤 organization/manage 재검증, bounded legacy Workflow 판정, permission·deployment·control-plane cleanup, retained usage/audit/trace·optimizer/routing·schedule/external-effect ledger 처리와 transaction-bound audit을 조율한다. 보존 row의 App/Workflow UUID는 권한 source가 아니며 durable organization provenance와 현재 actor 권한으로만 조회한다.
