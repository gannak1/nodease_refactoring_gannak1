...
- 현재 Gateway lifespan은 audit listener 등록, pgvector extension 확인, `Base.metadata.create_all()`, default user/provider/model seed, model pricing sync, SchedulerService 초기화를 수행한다. MBA-187 target에서는 deployed/local Gateway startup의 migration-managed table/enum `create_all()` 보정을 제거하고 Alembic readiness를 요구한다. Explicit demo/test bootstrap만 `create_all()`을 사용할 수 있다 ([ADR-0024](decisions/ADR-0024-distributed-schedule-dispatch-claim.md)).
