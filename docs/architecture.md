...
| Outbound operation registry / Guarded transport | LLM, Knowledge, Workflow 원격 파일과 Google OAuth/Gmail/GitHub/Slack fixed-SaaS 호출의 operation별 method·HTTPS/443·timeout·크기·content type·redirect profile과 revision을 소유한다. 승인 origin 안의 path/query만 허용하고 origin·Host authority를 network I/O 전에 확인하며, 모든 DNS 결과를 검증한 주소로 dial한 뒤 peer 일치와 원래 hostname TLS를 확인한다. Provider별 path/body 검증, credential 선택, 응답 의미, idempotency와 business retry는 adapter/application이 담당한다 ([ADR-0067](decisions/ADR-0067-production-https-and-operation-bound-outbound.md)). |
