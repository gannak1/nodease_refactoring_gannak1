...
RAG query embedding은 [ADR-0065](decisions/ADR-0065-rag-query-embedding-provider-capability.md)를 따른다. LLM Credentials는 기존 capability aggregate의 `query_embedding` purpose와 deployment-version/node/embedding-model별 explicit credential policy를 소유한다. Workflow의 `QueryEmbeddingExecutionRuntime` port는 authorized 후보의 distinct canonical embedding model마다 한 번만 capability와 provider attempt를 사용하며 query와 vector를 invocation-local로 유지한다. Generation credential, 실행 사용자, owner 또는 조직 기본 fallback은 허용하지 않는다. Capability runtime은 MBA-178의 authoritative egress adapter와 MBA-287의 durable provider operation ledger가 모두 연결된 경우에만 request byte를 전송한다.
