...
- External Action Credential은 GitHub/Slack secret을 organization-scoped encrypted resource로 분리한다. Gateway graph 저장, deployment preflight와 Workflow Engine runtime은 `ExternalActionCredentialUseResolver`를 공유해 provider, organization, credential 사용 주체의 `use`, lifecycle을 같은 규칙으로 해석한다. Interactive 실행은 명시적 user `execution_subject`, system schedule은 locked canonical deployment에서 구성한 user `credential_principal`만 Credential 사용 주체로 허용한다. Schedule principal을 executor, Knowledge subject 또는 audit actor로 승격하지 않는다. Preflight는 safe snapshot만 읽고, runtime은 provider I/O 직전에 fresh session으로 revision까지 재검증한다. Secret, raw Webhook URL, GitHub token은 graph·snapshot·trace·audit에 복제하지 않는다.
