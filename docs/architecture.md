...
| Security Alert Reconciler | Log System periodic Celery task | PostgreSQL watermark의 활성화 경계와 processor별 audit receipt로 실시간 처리 누락과 late commit을 찾고, 같은 organization·actor·action 범위에서 늦은 eligible audit 뒤의 최대 rule window를 다시 평가하며 기능 활성화 이전 audit은 backfill하지 않는다 |
