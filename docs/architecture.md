...
- **Workflow external node output raw payload**: 현재 Generic HTTP/Slack 성공 output의 `data`/`headers`와 GitHub comment의 기존 result field는 workflow downstream 및 실행 응답으로 전달될 수 있어 [PRD](PRD.md) NFR-004의 raw payload 응답 금지와 충돌한다. ADR-0035/MBA-190은 기존 shape를 회귀시키지 않으면서 새 idempotency identity/key와 durable trace/log 노출만 차단한다. Output allowlist/redaction과 downstream 호환 정책은 별도 보안 결정이 필요하다.
