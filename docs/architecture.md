...
| Outbound operation registry / Guarded transport | LLM, Knowledge와 Workflow 원격 파일의 operation별 method·HTTPS/443·timeout·크기·content type·redirect profile과 revision을 소유한다. 승인 origin 안의 path/query만 허용하고 origin·Host authority를 network I/O 전에 확인하며, 모든 DNS 결과를 검증한 주소로 dial한 뒤 peer 일치와 원래 hostname TLS를 확인한다. protocol별 SQL/command/listing, credential 선택과 business retry는 adapter/application이 담당한다 ([ADR-0067](decisions/ADR-0067-production-https-and-operation-bound-outbound.md)). |
...
| Workflow 노드 아웃바운드 | HTTP, FileExtraction, LLM, GitHub, Mail, Slack node | 실행 시점 외부 호출. Generic HTTP는 [ADR-0050](decisions/ADR-0050-workflow-generic-http-egress-boundary.md)의 application port, DNS-to-dial pinning과 Worker egress 정책을 적용한다. FileExtraction remote file과 LLM provider는 [ADR-0067](decisions/ADR-0067-production-https-and-operation-bound-outbound.md)의 operation profile과 guarded transport를 사용한다. Mail은 ADR-0031의 credential resolver와 IMAP egress gate를 적용한다. Slack은 provider 전용 API/Webhook adapter와 제한된 egress policy를 적용하고, durable claim/replay는 [ADR-0035](decisions/ADR-0035-external-effect-idempotency-boundary.md), provider 판정과 no-replay 정책은 [ADR-0037](decisions/ADR-0037-slack-dedicated-delivery-boundary.md)을 따른다. GitHub/Gmail/OAuth의 공통 transport 이관은 MBA-356 범위다. |
...
- 아웃바운드 통제: [ADR-0067](decisions/ADR-0067-production-https-and-operation-bound-outbound.md)에 따라 LLM generation·embedding/model discovery, Knowledge API source·remote document fetch/preview와 Workflow FileExtraction remote file은 immutable operation profile과 address-pinned guarded transport를 사용한다. Transport는 public HTTPS/443, bounded request/response, 원래 hostname TLS, `trust_env=false`, same-origin redirect와 safe error를 강제한다. Credential 저장 당시 endpoint snapshot은 current Provider catalog endpoint를 덮어쓰지 않는다. Workflow Generic HTTP는 [ADR-0050](decisions/ADR-0050-workflow-generic-http-egress-boundary.md)의 별도 HTTP 호환 계약을 유지한다. Mail IMAP 연결은 [ADR-0031](decisions/ADR-0031-mail-credential-reference-boundary.md)에 따라 public target, 허용 포트, DNS 결과를 검증하고 검증된 IP로 연결을 고정한다. Slack adapter는 commercial Web API 고정 endpoint 또는 정확히 검증된 incoming webhook host/path만 허용한다. Gateway와 Workflow Worker는 요청 또는 task 소비 전에 Mail 및 LLM keyring 형식과 active version을 각각 검증하되, credential secret을 사용하지 않는 Log System에는 keyring을 주입하지 않는다. GitHub/Gmail/OAuth와 잔여 adapter의 공통 guarded transport 이관은 MBA-356이 소유한다.
...
- **잔여 fixed-SaaS outbound surface**: GitHub, Gmail/OAuth와 일부 provider-specific adapter는 MBA-356 완료 전까지 [ADR-0067](decisions/ADR-0067-production-https-and-operation-bound-outbound.md)의 공통 operation transport에 모두 이관된 것으로 간주하지 않는다. Fixed HTTPS endpoint 검증과 기존 adapter별 guard는 유지하되 production readiness에서 잔여 direct client inventory를 확인한다.
