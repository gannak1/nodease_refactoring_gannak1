...
| Log System | `apps/log_system/` | Celery worker. audit outbox 전달, trace/log 계열 비동기 처리와 bounded Agent Builder L2 cache retention purge |
...
| Redis | 컨테이너/chart | Celery broker/result, Pub/Sub와 Agent Builder intent cache의 fail-open 내부 저장소. ADR-0063의 strict contract/codec/port, ADR-0073의 exact-token 경계 및 ADR-0074의 safe literal admission과 함께 serving Redis/config/coordinator/lifecycle composition까지 구현됐다. Cache serving은 기본적으로 요청되며 `scripts/dev-local.ps1`는 cache 전용 Redis DB, ephemeral HMAC key/version으로 local Gateway를 cache-on으로 시작한다. 전용 Redis URL·HMAC key/version·bounded configuration이 불완전한 직접 실행은 Celery Redis로 fallback하지 않고 cache만 비활성화한다. `NODE_ENV=production|staging` serving은 전용 URL, HMAC/bounded configuration, 외부 운영 evidence 뒤 운영자가 설정한 readiness attestation을 모두 요구하며, 그 전까지는 비활성이다. 런타임은 별도 evidence 저장소를 조회하지 않는다. 실제 instance/secret wiring/용량·eviction/장애·monitoring·rollback rollout은 후속 운영 범위다. |
...
| Celery Beat Scheduler | Shared allowlisted periodic schedule을 singleton 프로세스로 실행해 Audit Outbox 배송, Security Alert reconciliation, due/stale KC sync recovery, Knowledge document ingestion recovery와 Agent Builder L2의 5분 bounded retention catch-up purge를 각 전용 queue에 발행한다. L2 purge task는 최대 5개의 1,000-row batch를 처리하고 expired encrypted cache row만 hard-delete하며 cache audit/replay source를 만들지 않는다. Helm은 `Recreate` singleton Deployment를 사용해 rollout 중 중복 scheduler를 피한다. Production Knowledge worker가 활성화된 profile은 이 recovery scheduler도 반드시 활성화한다. |
...
| Agent Builder Intent Plan Cache | **Accepted implementation.** Strict contract/codec/port, `intent-normalizer-v2`, Redis/single-flight coordinator와 Gateway lifecycle composition이 구현됐다. 모든 non-empty safe request는 secret/redaction, truncation, explicit value, malformed quote와 ambiguous modify target gate를 통과하면 eligible이다. Catalog 단일 exact token만 canonicalize하고 Catalog 미등록 자연어와 지원하지 않는 node 이름은 ordered literal로 보존한다. Cache serving은 기본 요청이며 `scripts/dev-local.ps1`는 cache-on으로 시작한다. 직접 실행에서 cache 전용 Redis URL·HMAC key/version·bounded configuration이 불완전하면 cache만 비활성화하고 Celery Redis로 fallback하지 않는다. Production/staging은 명시적 readiness attestation과 운영 evidence 전까지 비활성이다. Cache key는 organization/user scope, planner runtime fingerprint, generation mode, intent signature, selected target identity, Knowledge candidate fingerprint와 normalizer/cache/registry contract version만 HMAC 입력으로 사용한다. 전체 workflow topology와 raw graph configuration은 key에 넣지 않고 hit revalidation에만 사용한다. Cache value는 graph, UUID, operation, credential, parameter 값과 Knowledge handle을 저장하지 않으며, hit에도 현재 model/credential 권한, target, Knowledge 후보, Catalog validation과 GraphMutation/CAS/acknowledgement를 다시 수행한다. Redis 오류와 version mismatch는 audit replay가 아니라 기존 Planner 경로로 fail-open한다 ([ADR-0063](decisions/ADR-0063-agent-builder-deterministic-intent-plan-cache.md), [ADR-0073](decisions/ADR-0073-agent-builder-cache-exact-token-normalization-amendment.md), [ADR-0074](decisions/ADR-0074-agent-builder-cache-safe-literal-normalization-amendment.md)). |
| Agent Builder Private Dense Semantic Cache | **JEO-8 implemented, default-disabled.** ADR-0076은 L1과 L2가 모두 명시적 exact miss인 target-independent `new_workflow`에만 private `organization_id + user_id` pgvector candidate assist를 둔다. L2 `None|invalid|unavailable`과 non-read cohort는 semantic을 시작하지 않는다. Selected-target와 modify/replace 요청은 semantic I/O·index·serving에서 제외되고 decoded candidate plan도 같은 eligibility를 verifier 전에 다시 확인한다. Candidate는 score만으로 serve되지 않고 provider-call-free verifier와 existing current rehydrator를 모두 통과해야 하며, query embedding provider call이 남을 수 있는 Planner-free serving은 별도 default-disabled flag다. Provider 입력은 잘리지 않은 최대 240-code-point versioned transient `SemanticQueryProjectionV1`과 purpose-bound opaque execution binding만 허용하고 더 긴 projection은 Planner로 fail-closed하며, projection version을 index/search compatibility metadata로 저장한다. Provider-backed verifier는 query binding을 재사용하지 않고 별도 verification binding을 요구한다. Semantic external boundary 뒤에는 cancellation/version fence를 재확인하고 candidate rehydration이 연 service read transaction은 serving/Planner 전에 정리한다. Tests inject fakes, but no live production adapter is composed before an approved Agent Builder `query_embedding` capability/profile and durable usage binding. L2-only strict stored receipt는 Redis DTO를 바꾸지 않고 semantic append에 actual parent ID/expiry/write kind를 전달한다. Semantic index는 safe embedding/version metadata와 L2 record FK만 보관하며 append SQL은 current `IntentPlanningContext.organization_id` plus receipt parent ID/expiry를 fence한다. Insert/unexpired conflict는 missing child를 만들고 existing conflict를 no-op하며 expired replacement는 purged child insert 또는 older-child conditional update만 허용한다. Existing L2 retention task는 process-lifetime disable 없이 매 실행 만료 child를 격리 purge한 뒤 parent cascade를 수행하므로 failed best-effort replacement가 old embedding retention을 연장하지 않는다. GraphRAG, template recommendation, shared cache, public API와 Client UI는 범위 밖이다. |
...
| Public Conversation Lifecycle | Same-origin iframe의 public Session·Access Grant 생성, transcript 조회, close/reset/privacy delete, purge receipt 조회와 transaction-bound canonical audit를 조율한다. 기본 비활성 feature를 명시적으로 활성화하고 독립 key와 backup erasure 전제를 충족한 환경에서만 composition한다. Network admission은 공통 trusted-proxy resolver가 canonical client network로 정규화하며 unknown identity를 fail-closed한다. |
...
| Public Replay Retention | Memory가 만료 판정과 bounded ciphertext 삭제 정책·repository를 소유한다. Gateway/Log System runtime image는 `apps/memory`를 포함하고 기본 활성 singleton Helm Beat가 shared schedule을 발행한다. 공용 Log queue/worker는 주기 task의 실행 host일 뿐 Conversation state나 retention policy의 source of truth가 아니다. |
