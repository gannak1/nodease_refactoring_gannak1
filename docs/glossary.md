...
| Agent Builder Intent Plan Cache | ADR-0063과 ADR-0075의 Gateway 내부 cache 계층이다. L1 feature flag의 기본 요청값은 enabled지만 전용 Redis·HMAC·bounded 설정이 불완전하면 serving은 안전하게 비활성화된다. 결정적으로 동등하다고 검증 가능한 요청은 먼저 Redis L1에서 cache-safe typed intent plan을 재사용하고, L1 miss인 strict allowlisted organization만 encrypted PostgreSQL L2를 조회한다. L2는 생성 시점부터 30일간 plan만 보관하며 current revalidation·rehydration과 L2 commit 뒤에만 L1 promotion을 허용한다. 완성 graph, protected resource reference, authorization decision과 request replay data를 저장하지 않으며 Redis/L2 모두 source of truth나 audit replay source가 아니다. 오류는 Planner cold miss로 fail-open하고 production/staging serving은 cache 전용 Redis와 외부 readiness evidence 뒤 운영자 attestation이 필요하다. |
