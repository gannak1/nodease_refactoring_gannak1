...
| Provider Execution Capability | LLM Credentials domain이 authoritative하게 발급·검증하는 short-lived opaque capability. Canonical runtime scope, server-derived credential principal, credential permission decision, verified relation/egress/pricing revision, invocation/admission/provider-attempt, main/summary purpose, token/cost cap과 expiry를 identity/revision에 고정한다. Memory, Workflow와 Budget은 opaque identity/revision과 자기 operation binding만 소비하며 scope를 재정의하지 않는다. |
...
| External Action Credential | Organization이 관리하는 GitHub/Slack action 인증 resource. Workflow graph에는 opaque `credential_id`만 저장하며 secret은 `external_action_credentials`에 versioned envelope로 암호화한다. 조회·사용·관리는 `read/use/manage`로 분리하고 provider call 직전에 execution subject, provider, 상태, revision을 재검증한다. |
